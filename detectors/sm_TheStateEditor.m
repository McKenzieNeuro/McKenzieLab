...
if isempty(baseName)
    ...
    if length(xmlBase) == 0
        ...
        xmlBase = xmlBase(1);
        choice = questdlg(['No basename entered, use ', ...
            xmlBase.name(1:(end - 4)), ' as file basename?'],'No Basename','Yes','Cancel','Yes');
...
if FileExistsIn([baseName,'.eegstates.mat'])
    ...
    if supressLoadGUI == 0
        ...
        mOptions = 'None|Load From .whl file (head tracking)|Load from eeg ch(s) (accelerometer/motion pad)|Load from eeg ch(s) (MEG)|Load from .mat file|Load from basename_auxiliary.dat accelerometer file';
    ...
    if ~exist('rawEeg', 'var')
        ...
        try
            ...
            eeg1 = LoadBinaryIn([baseName, suffix], 'channels', Chs, 'nChannels', nCh,'frequency',xml.lfpSampleRate)';
            eeg1 = single(eeg1);
        end
        disp('Done.');
        for i = 1:length(Chs)
    ...
    if ~exist('motion', 'var')
        ...
        switch(mIn)
            ...
            case 6
                 ...
                 disp(['Loading motion data from ' baseName '_auxialiary.dat']);
                motion = LoadFromAuxiliary(baseName, fspec{1}.to);
...
function motion = LoadFromAuxiliary(baseName,tos)


fName = [baseName '_auxiliary.dat'];
fxml =  [baseName '_auxiliary.xml'];
sizeInBytes = 2; %
if ~exist(fName,'file')
...
if ~exist(fxml,'file')
    error([fxml ' not found, you need to create xml'])
end

fInfo = dir(fName);
[xml, rxml] = LoadXml(fxml);


inFs = xml.SampleRate;
nbChan = xml.nChannels;
...
sampleRatio =inFs/FsOut ;


%chunk should be even multiple of sampleRatio
chunksize = 1e6; % depends on the system... could be bigger I guess
...
fprintf('Extraction of accelerometer begun \n')
motion = nan(length(tos),1);
...
for ibatch = 1:nbChunks
    ...
    if mod(ibatch,10)==0
        ...
        dat = fread(fidI,nbChan*(chunksize),'int16');
        dat = reshape(dat,[nbChan chunksize]);
      %  downDat = dat(:,sampleRatio:sampleRatio:end);
        ts = [1:size(dat,2)]/inFs +  (ibatch-1)*(chunksize/inFs);
        dat = dat(:,ts<max(tos));
        ts = ts(ts<max(tos));
    [~,b] = histc(ts,[tos]);
    b1 = b-min(b)+1;
    nS = range(unique(b1))+1;
   downDat = nan(size(dat,1),nS);
    for i = 1:size(dat,1)
        tmp = accumarray(b1',dat(i,:)',[nS 1],@nanmean,nan);
  
        %downDat(i,:) = tmp(1:size(downDat,2));
          downDat(i,:) = tmp;
    end
   
    motion(unique(b(1:end))') = sqrt(sum(downDat.^2));
end
...
if ~isempty(remainder)
    ...
    dat = reshape(dat,[nbChan remainder]);
     ...
     downDat = dat(:,1:sampleRatio:end);
        ts = [1:size(dat,2)]/inFs +  (ibatch-1)*(chunksize/inFs);
    [~,b] = histc(ts,[tos;inf]);
    b1 = b-min(b)+1;
    for i = 1:size(dat,1)
          tmp = accumarray(b1',dat(i,:)',[],@mean,nan);
        
        downDat(i,:) = tmp(1:size(downDat,2));
    end
    
    motion(unique(b(1:end-1))) = sqrt(sum(downDat.^2));
...
motion = motion-nanmedian(motion);
end
