function [ripples] = sm_FindRipples(varargin)
...
%    channel      	Ripple channel to use for detection (base 0)
%
%    =========================================================================
%     Properties    Values
%    -------------------------------------------------------------------------
...
addParameter(p,'durations',[30 200],@isnumeric)
...
addParameter(p,'EMG',false,@islogical);
addParameter(p,'saveMat',true,@islogical);
addParameter(p,'saveFMA',true,@islogical);
...
if isstr(varargin{1})  % if first arg is basepath
    ...
    lfp = sm_GetLFP(p.Results.channel,'basepath',p.Results.basepath,'basename',basename,'nChannels',nChannels,'samplingRate',frequency);%currently cannot take path inputs
%     lfp = sm_GetLFP(1:32,'basepath',p.Results.basepath,'basename',basename,'nChannels',nChannels,'samplingRate',frequency);%currently cannot take path inputs
%     u = median(double(lfp.data),2);
%     lfp.data = lfp.data(:,p.Results.channel);
%     lfp.data = double(lfp.data)-u;
    signal = bz_Filter(double(lfp.data),'filter','butter','passband',passband,'order', 3);
    
    if ~isempty(p.Results.noise)
        noise = sm_GetLFP(p.Results.noise,'basepath',p.Results.basepath,'basename',basename,'nChannels',nChannels,'samplingRate',frequency);%currently cannot take path inputs
        noise = noise.data;
        noise = bz_Filter(double(noise),'filter','butter','passband',passband,'order', 3);
        
        % squaredSignal = signal.^2 - noise.^2;
    else
        noise =[];
    end
...
%take into account noise channel when excluding epoch
...
if ~isempty(noise)
    ...
    if any(keep)
        keep = keep & normalizedSquaredNoise<10;
    else
        keep =  normalizedSquaredNoise<10;
    end
end

[normalizedSquaredSignal,sd] = unity(Filter0(window,sum(squaredSignal,2)),sd,squaredSignal>0 & keep);
...
if any(restrict)
    [status, interval] = InIntervals(firstPass(:,1),restrict*frequency);
...
detectorinfo.detectiondate = datetime;
...
if p.Results.saveFMA
    filename = [basepath filesep basename '.evt.rip'];
    sm_SaveRippleEvents(filename,ripples,channel)
end
