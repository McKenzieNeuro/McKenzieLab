...
ops.rix  = randsample(1:length(group),round(length(group)/2));
...
test_idx = ops.N+1:size(training,1);
test = training(test_idx,:);
ses_test = ses1(test_idx,:);
kp = ses_test(:,3)>.8;
[pred,score] = predict(rusTree,test(kp,:));


%get confusion matrix
actual_Y = group(test_idx);
predicted_Y = pred;
C = confusionmat(actual_Y(kp),predicted_Y);
figure
imagesc(C./sum(C,2))
%%
