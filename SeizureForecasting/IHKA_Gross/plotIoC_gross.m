...
%%
clear bb
[~,b] = histc(-all_tim2seiz, [0 logspace(log10(1),log10(5e4),100) inf]);
for i = 1:5
bb(i,:) = accumarray(b(b>0),all_pred(b>0,1)==i,[max(b) 1],@nanmean,nan);
end
%%
...
lbl = {'>3600 s','100-3600 s','10-100 s','0-10 s','ictal','post-ictal'};
 
fprintf('\n===== n = %d seizures, %d sessions =====\n', N, numel(unique(ID_IoC(kp,1))));
 
fprintf('\n--- Table 1: discrimination ---\n');
fprintf('%-12s %14s %14s %14s %14s %7s %14s\n', ...
        'class','prevalence','recall','precision','F1','lift','ROC-AUC');
lift = nanmean(precision(kp,:))./nanmean(prev(kp,:));
for jj = 1:nGr
    fprintf('%-12s %6.4f±%-6.4f %6.3f±%-6.3f %6.3f±%-6.3f %6.3f±%-6.3f %7.2f %6.3f±%-6.3f\n', ...
      lbl{jj}, nanmean(prev(kp,jj)),SEM(prev(kp,jj)), ...
      nanmean(sensitivity(kp,jj)),SEM(sensitivity(kp,jj)), nanmean(precision(kp,jj)),SEM(precision(kp,jj)), ...
      nanmean(f1(kp,jj)),SEM(f1(kp,jj)), lift(jj), ...
      nanmean(AUC(kp,jj)),SEM(AUC(kp,jj)));
end
% per-seizure lift, for the reported value (mean of ratios, not ratio of means)
liftSz = precision(kp,:)./max(prev(kp,:),eps);
fprintf('per-seizure lift (median): ');
fprintf('%.2f ', nanmedian(liftSz)); fprintf('\n');
 
fprintf('\n--- Table 2: vs permutation null ---\n');
fprintf('%-12s %14s %14s %12s %12s\n','class','Brier','Brier skill','n p<.05','frac');
for jj = 1:nGr
    k = sum(p_sz(kp,jj)>.95);
    fprintf('%-12s %6.3f±%-6.3f %6.3f±%-6.3f %12d %11.3f\n', lbl{jj}, ...
      nanmean(BS(kp,jj)),SEM(BS(kp,jj)), nanmean(RSS(kp,jj)),SEM(RSS(kp,jj)), k, k/N);
end
 
fprintf('\n--- IoC (mean confusion, improvement over chance) ---\n');
disp(nanmean(IoC(:,:,kp),3));
fprintf('(NOTE: IoC scales inversely with predicted-class marginal.\n');
fprintf(' Not comparable across classes or datasets. Use AUC/lift.)\n');
 
fprintf('\n--- class-1 AUC by latency ---\n');
s1lbl = {'1-2 h','2-4 h','4-8 h','>8 h'};
for b = 1:nS1
    fprintf('%-8s nSz=%3d  AUC = %.3f ± %.3f\n', s1lbl{b}, sum(N1(kp,b)>0), ...
            nanmean(AUC1(kp,b)), SEM(AUC1(kp,b)));
end
 horizonSec = 10;
fprintf('\n--- alarm characteristics (class %d, %ds horizon) ---\n', alarmClass, horizonSec);
fprintf('alarms/hour        : %.1f ± %.1f\n', nanmean(alarmsPerHour(kp)), SEM(alarmsPerHour(kp)));
fprintf('block length (s)   : %.2f ± %.2f\n', nanmean(blockLenMean(kp)), SEM(blockLenMean(kp)));
fprintf('time in warning    : %.4f ± %.4f\n', nanmean(TiW(kp)), SEM(TiW(kp)));
obs = logical(sens_block(kp)); pch = chance_sens(kp);
ok  = ~isnan(pch); obs = obs(ok); pch = pch(ok);
fprintf('sensitivity        : %.3f (%d/%d)\n', mean(obs), sum(obs), numel(obs));
fprintf('matched chance     : %.3f (expected %.1f)\n', mean(pch), sum(pch));
fprintf('lift               : %.2f\n', mean(obs)/mean(pch));
pmf = 1; for ii = 1:numel(pch), pmf = conv(pmf,[1-pch(ii) pch(ii)]); end
kdet = sum(obs);
fprintf('Poisson-binomial   : z = %.1f, p = %.3g\n', ...
        (kdet-sum(pch))/sqrt(sum(pch.*(1-pch))), sum(pmf(kdet+1:end)));
fprintf('signed-rank        : p = %.3g\n', signrank(double(obs),pch,'tail','right'));
 
 
%---- MAP rate and per-class SCORE as a function of true time-to-seizure ----
edges = [0 logspace(log10(1),log10(5e4),100) inf];
[~,b] = histc(all_tim2seiz, edges);
uS = unique(all_IDs);
bb = nan(nGr,numel(edges)-1,numel(uS));   % MAP rate
ss = nan(nGr,numel(edges)-1,numel(uS));   % mean score
for s = 1:numel(uS)
    q = b>0 & all_IDs==uS(s);
    for i = 1:nGr
        bb(i,:,s) = accumarray(b(q), all_pred(q,1)==i,  [numel(edges)-1 1], @nanmean, nan);
        ss(i,:,s) = accumarray(b(q), all_pred(q,1+i),   [numel(edges)-1 1], @nanmean, nan);
    end
end
nContrib = squeeze(sum(~isnan(bb(1,:,:)),3));
 
band = 77:98; tail = 100:101;                  % PRE-SPECIFY these before testing
fprintf('\n--- band (bins %d-%d) vs tail (bin %d) ---\n', band(1), band(end), tail);
fprintf('%-12s %10s %10s %10s %10s %10s\n','class','MAPband','MAPtail','p','SCOREband','SCOREtail');
for i = 1:nGr
    mb = squeeze(nanmean(bb(i,band,:),2)); mt = squeeze(nanmean(bb(i,tail,:),2));
    sb = squeeze(nanmean(ss(i,band,:),2)); st = squeeze(nanmean(ss(i,tail,:),2));
    fprintf('%-12s %10.3f %10.3f %10.4f %10.3f %10.3f\n', lbl{i}, ...
        nanmean(mb), nanmean(mt), ranksum(mb,mt), nanmean(sb), nanmean(st));
end
fprintf('sessions contributing, bins 90-101: '); fprintf('%d ', nContrib(90:101)); fprintf('\n');
 
%% ---- local function ----
function [on,off] = mergeblocks(v, gap)
v = v(:); d = diff([0; v; 0]);
on = find(d==1); off = find(d==-1)-1;
if isempty(on), on=[]; off=[]; return; end
keep = true(size(on));
for b = 2:numel(on)
    if on(b)-off(b-1)-1 <= gap, keep(b) = false; end
end
ki = find(keep); on = on(keep); off2 = zeros(size(on));
for b = 1:numel(ki)
    if b < numel(ki), off2(b) = off(ki(b+1)-1); else, off2(b) = off(end); end
end
off = off2;
end
