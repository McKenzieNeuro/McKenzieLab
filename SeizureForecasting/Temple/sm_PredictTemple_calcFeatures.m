function features = sm_PredictTemple_calcFeatures(fname,tim,ops)
...
% figure out which channel goes where
...
fils = ops.txtfils;
ch_txt  = fils(contains(fils,fname));
...
clear chOrder
for j = 1:24
    tmp =  find(cellfun(@any,regexpi(lab',channel{j})),1,'first');
    if ~isempty(tmp)
        chOrder(j) = tmp;
    else
        chOrder(j) = 0;
...
for ch = 1:nCh_raw
    ...
    if chOrder(ch)~=0
        txtFil = ch_txt{chOrder(ch)};
        powerFil = [txtFil(1:end-3) 'dat'];
        
        if exist(powerFil) && exist(txtFil)
            ...
            features(:,ch) = [mean(tmp(:,2:2:40))/reScalePower];
