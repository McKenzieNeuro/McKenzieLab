...
%EEG1 -> Contralateral cortical
%EEG2 -> Ipsilateral cortical
%EEG3 -> Ipsilateral hippocampal
%EEG4 -> Contralateral hippocampal

load('R:\IHKA_Scharfman\features\features.mat','dat','ops','sesID','sessions')

% extract from cell arrays


all_features = cell2mat(dat');

%get sessionID info for each observation (row in all_features)
ses = cell2mat(sesID');

%get which time interval each observation came from
group = cell2mat(cellfun(@(a,b) b*ones(size(a,1),1),dat,num2cell(1:length(dat)),'uni',0)');

% get some meta data about the featurization
freqs = ops.freqs;
bins = ops.bins;
%%

% get information about the timing of each seizure
% (stats will be run per seizure)
...
TSname1 = 'seizure st';
% find nearest seizure (will be unit of analysis)
clear seizure_start
tot = 0;
for i = 1:length(sessions)
    TSdata = readtable(sessions{i,1});
    TSdata = table2cell(TSdata);
    tmp = cell2mat(TSdata(cellfun(@any,regexpi(TSdata(:,6),TSname1)),4));
    
    seizure_start{i}(:,1) = tmp;
    seizure_start{i}(:,2) = (1:length(tmp))+tot;
    tot = tot+length(tmp);
...
[~,subj] = fileparts(sessions(:,2));
subj = strrep(subj,'Amigo_Cre_','AC');
subj = strrep(subj,'78a','78');
subj = strrep(subj,'75a','75');
subj = cellfun(@(a) a(1:4),subj,'UniformOutput',false);
[a,b,subjID] = unique(subj);


%%
% for each observation we will link that time point to a seizure
...
% get mean feature info per seizure
nsz = max(ses(:,3));
trainU = nan(nsz,260,6);
for i = 1:nsz
    
    for j = 1:6
        kp = ses(:,3) == i & group==j;
        
        trainU(i,:,j) = nanmean(all_features(kp,:));
...
%plot PSD for each channel for each interval
close all
figure
...
tt = nan(6,10,length(all_features));
for i = 1:length(all_features)
    
    tt(:,:,i) =  reshape(all_features(i,idx),length(apr),length(idx1));
    
end


%% plot phase amplitude per epoch
close all
...
%plot coherence for each time interval
...
for j = 1:6
    ...
    for i = 1:6
        ...
        kp = group==j;
        %kp=kp1;
        semilogx(freqs(1:20), nanmean(all_features(kp ,ix)),'linewidth',3,'color',col{i})
        hold on
...
%calculate mixed effects ANOVA for each feature to determine which varies
%most by temporal epocj tp seizure (not counting sz and postictal)


Interval = (upSample(1:4,nsz));
colNames = {'SeizureID','Animal', 'Interval'};
featureColNames = arrayfun(@(x) sprintf('Feature_%d', x), 1:260, 'UniformOutput', false);
allColNames = [colNames, featureColNames];
variableTypes = ['categorical' 'categorical' 'categorical' repmat({'double'}, 1, 260)];
...
% plot F-ratio for frequency power
...
% plot F-ratio for phase/amplitude
