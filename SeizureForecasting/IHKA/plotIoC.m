    ...
    for i = 1:length(ix)
        ...
        for jj = 1:6
            if any(actual_Y==jj) && ~all(actual_Y==jj)
                [X,Y,T,AUC(i,jj)] = perfcurve(actual_Y==jj,confLabel_Y(:,jj),1);
            else
                AUC(idx,jj) = nan;
            end
            [f1(idx,jj), sensitivity(idx,jj), specificity(idx,jj)] = computeClassificationMetrics(actual_Y==jj,predicted_Y==jj);
