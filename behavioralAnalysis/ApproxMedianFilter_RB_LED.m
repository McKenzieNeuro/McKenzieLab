function [whl,in,threshF,out] = ApproxMedianFilter_RB_LED(file,outname,varargin)
...
[XX,YY] = meshgrid(1:size(fr,2),1:size(fr,1));
out = inpolygon(X(:),Y(:),x,y);
out = repmat(reshape(out,[size(fr,1) size(fr,2)]),[1 1 3]);
...
fr_bw = rgb2gray(fr);
label         = repmat(logical(fr_bw>0),[1 1 3]) & in & ~out;
mask(label)   = fr(label);
mask(~label)  = 0;
...
for i = Fint:readerobj.NumberOfFrames
    ...
    %     keyboard
    %%% Label Color Mask
    label         = repmat(logical(fr_bw>threshF),[1 1 3]) & in & ~out;
    mask(label)   = fr(label);
    mask(~label)  = 0;
    ...
    if Nl > 0
        ...
        if (~isempty(mR) && ~isempty(mB)) && mR > 200
            
            Rr  = round(mean(a(bR).PixelList,1));
        end
        
        
        if  (~isempty(mR) && ~isempty(mB)) &&  mB > 200
