...
% Block schedule 
firstBL = 90; % Pre-stim baseline duration in minutes
stimDur = 60; % Duration of stim period in minutes
finalBL = 90; % Post-stim baseline duration in minutes

% Stim parameters
stimI = 100; % Stim 1 intensities in uA, randomly selected
thresholdTD = 3; % Theda/delta threshold for stim
    ...
    elseif labindex == intanReader        
        ...
        while getTime <= seshStart+firstBL+stimDur
            ...
            if any(HaveByte >= waveformBytesBlocks)
                ...
                %Track which block chunk has just come in. If there have already been all blocks plotted, then reset to 1.
                chunkCount = chunkCount+1;
                if chunkCount > blocksPerRead
                    chunkCount = 1;
                end
                ...
                ampTimewindow(1,end+numDsFrames) = amplifierData;          
                ...
                if readCount >= nextISI
                    ...
                    thetaAmp = mean(abs(hilbert(theta)));
                    deltaAmp = mean(abs(hilbert(delta)));
                    theta_delta = thetaAmp/deltaAmp;

                    if theta_delta <= thresholdTD
                        labSend(1,intanSender);
                        send(textOut,'Stim');
                        nextISI = datasample(bankISI,1);
                        readCount = 0;
                    else
                        labSend(0,intanSender);
                    end
                end
...
for i = 2:numFolders+1
    ...
    for ii = 1:size(data,1)
        ...
        theta_delta = zeros(1,length(1:dSF:L));
        count = 1;
        for i3 = 1:dSF:L
            ...
            try
                Y_theta = fft(theta(1,tWinS:tWinF));
                Y_delta = fft(delta(1,tWinS:tWinF));
            catch
                Y_theta = fft(theta(1,tWinS:L));
                Y_delta = fft(delta(1,tWinS:L));
            end
            l = length(Y_theta);
            P2_Y_theta = abs(Y_theta/l);
            P2_Y_delta = abs(Y_delta/l);
            P1_Y_theta = P2_Y_theta(1:l/2+1);
            P1_Y_delta = P2_Y_delta(1:l/2+1);
            theta_delta(1,count) = mean(P1_Y_theta(:,P1_Y_theta>1))/mean(P1_Y_delta(:,P1_Y_delta>1)); 
            count = count+1;
        end
        recInfo{i,5}{ii,2} = theta_delta;
    end   
...
theta = BandpassFilter(double(d),dSF,[5 12]);
power_theta = InstAmplitude(theta);
delta = BandpassFilter(double(d),dSF,[1 4]);
...
theta_delta = power_theta./power_delta;

hold on
plot(theta_delta)
xline(timeStamps(1,1),'LineWidth',3)
xline(timeStamps,'LineWidth',3)
