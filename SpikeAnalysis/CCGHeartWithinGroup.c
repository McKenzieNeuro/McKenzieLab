...
/* This is a bare-bones C program whos purpose is to compute
   ...
   CONDITIONS binary file containing N unsigned ints giving the condition markers. Computes across conditions  Don't use zero!
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
	...
	unsigned int CenterSpike, Mark1, Mark2, Bin, Group1,Group2;
	...
	CountArraySize = nMarks * nMarks * nBins * nGroups;
     nCountArraySize = nMarks * nGroups;
	...
	for(CenterSpike=0; CenterSpike<nSpikes; CenterSpike++) {
        ...
        Group1 = Groups[CenterSpike];
		 ...
		 nCountIndex = nMarks * (Group1-1) + Mark1-1;
         NCount[nCountIndex]++;
		...
		for(SecondSpike=CenterSpike-1; SecondSpike>=0; SecondSpike--) {
			...
			CountIndex = nBins*nMarks*nMarks*(Group1 - 1) + nBins*nMarks*(Mark1-1) + nBins*(Mark2-1) + Bin;
#ifdef CHECK
