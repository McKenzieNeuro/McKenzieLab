...
params.addParamValue('lambda',[.005 0 0],@isvector);
params.parse(varargin{:});
    ...
    for n = 1:N
        ...
        for jj = 1:num_rows
            ...
            for i = 1:innerIterMaximum
                ...
                m_row = m_rowNEW;
                % 
                % if n == 1   % only apply to neuron mode
                %     % lambda should be 1 x N (or N x 1) with one penalty per neuron
                %     m_row = max(m_row - lambda(jj), 0);  % safe soft-thresholding
                %     M{n}(jj,:) = m_row;
                % end
