function manifold_flow_gui3(ses)
...
% ══════════════════════════════════════════════════════════════════════════
    ...
    function do_redraw()
        ...
        for m = 1:nM
            ...
            % ── Flow field: K-space velocity from Znorm, K-space weighted J ─
            % Znorm is already rate-free so velocities need no further decomposition.
            rip_sp_idx = find(kp_sp);
            Xp = []; Vp = []; uXp = [];
            for rr = 1:Rsp
                tr = squeeze(D.Znorm(:,:,rip_sp_idx(rr)));  % K x T, normalized
                u0 = usp(rr,:);
                for t = 2:D.T-1
                    v   = (tr(:,t+1) - tr(:,t-1)) / 2;
                    Xp  = [Xp,  tr(:,t)]; %#ok
        ...
        % ── Stim trajectories: J*(z_t - z_mean) using Znorm ─────────────
        % Znorm is consistent with UMAP space — J is well-defined.
        all_traj_data = {};
        for m2 = 1:nM
            cl2    = sel_cl(m2);
            n_st2  = sel_nstim(m2);
            ...
            % Build Xp2/uXp2 from Znorm
            Xp2 = []; uXp2 = [];
            for rr2 = 1:Rsp2
                tr2 = squeeze(D.Znorm(:,:,rip_sp_idx2(rr2)));  % K x T
                u0  = usp2(rr2,:);
                for t2 = 2:D.T-1
                    Xp2  = [Xp2,  tr2(:,t2)]; %#ok
                    uXp2 = [uXp2; u0];         %#ok
            ...
            % Variance-weighted random sampling
            tvar2 = zeros(Rst2,1);
            for rr = 1:Rst2
                tr = squeeze(D.Znorm(:,:,rip_st_idx2(rr)))';
                tvar2(rr) = sum(var(tr,0,1));
            end
            tvar2_w   = tvar2 / (sum(tvar2) + 1e-12);
            ...
            for rr = show_rr2'
                % Stim ripple already normalized in Znorm
                tr_rn  = squeeze(D.Znorm(:,:,rip_st_idx2(rr)))';  % T x K
                u_anc  = ust2(rr,:);
                z_mean = mean(tr_rn,1);

                % Local Jacobian weighted by K-space proximity
                ...
                all_traj_data{end+1} = struct('u_traj', u_traj_raw); %#ok
            end
        end
