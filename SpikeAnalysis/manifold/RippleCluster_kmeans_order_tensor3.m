                ...
                for ii = 1:optimal_k
                    ...
                    if sum(kp_base)>5
                        ...
                        if sum(kp_stim)>5 && sum(kp_held_out)>5
                            ...
                            % -------------------------------------------------
                            % GEODESIC DISTORTION (LANDMARK METHOD — CONNECTIVITY SAFE)
                            % -------------------------------------------------

                            kNN = 10;

                            % ----- reference graph (training data only) -----
                            Dsp = squareform(pdist(X,'cosine'));
                            ...
                            % Geodesic distances from training points to landmarks
                            Dref = distances(G, 1:N, landmarks);   % (N x nLand), may contain Inf

                            % -------------------------------------------------
                            % Attach held-out points (Y) to fixed graph
                            % -------------------------------------------------

                            Dx_Y = pdist2(Y, X, 'cosine');   % (M x N)
                            [~, idx_Y] = sort(Dx_Y, 2);
                            ...
                            for i = 1:M
                                nbrs = idx_Y(i, 1:kNN);  % nearest neighbors in training set

                                % propagate distances through valid neighbors only
                                % ignore neighbors that cannot reach a given landmark
                                d_candidates = Dx_Y(i,nbrs)' + Dref(nbrs,:);  % (kNN x nLand)

                                % take min over finite paths only
                                d_candidates(~isfinite(d_candidates)) = NaN;

                                Yg(i,:) = min(d_candidates, [], 1, 'omitnan');
                            end

                            % -------------------------------------------------
                            % Attach stimulated points (Z)
                            % -------------------------------------------------

                            Dx_Z = pdist2(Z, X, 'cosine');
                            [~, idx_Z] = sort(Dx_Z, 2);
                            ...
                            for i = 1:M2
                                ...
                                d_candidates = Dx_Z(i,nbrs)' + Dref(nbrs,:);
                                d_candidates(~isfinite(d_candidates)) = NaN;

                                Zg(i,:) = min(d_candidates, [], 1, 'omitnan');
                            end

                            % -------------------------------------------------
                            % Compare geometry (finite distances only)
                            % -------------------------------------------------

                            q = linspace(0.1,0.9,9);

                            % reference: only landmark pairs with finite paths
                            D_LL = Dref(landmarks,:);   % (nLand x nLand proxy)
                            d_ref_all = pdist(D_LL);
                            d_ref = quantile(d_ref_all(isfinite(d_ref_all)), q);

                            % held-out
                            d_ho_all = pdist(Yg);
                            d_ho = quantile(d_ho_all(isfinite(d_ho_all)), q);

                            % stimulation
                            d_st_all = pdist(Zg);
                            d_st = quantile(d_st_all(isfinite(d_st_all)), q);
