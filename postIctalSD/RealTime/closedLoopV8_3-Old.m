...
stimParam(1).config(1).subject = 'LPP 1.0'; % Subject in box 1
stimParam(1).config(2).subject = 'LPP 2.0'; % Subject in box 2
...
stimParam(2).config(2).chan = [6,nan];
stimParam(2).config(3).chan = [nan,nan];
stimParam(2).config(4).chan = [nan,nan];

% Amplitudes in uA.
stimParam(2).config(1).amp = 123;
stimParam(2).config(2).amp = 18;
...
% Prediction:
usePrediction = 1;
modelPath{1,1} = 'R:\McKenzieLab\Analysis\SeizureForecasting\IHKA_rat_RF\modelPCP4.mat';
modelPath{2,1} = 'R:\McKenzieLab\Analysis\SeizureForecasting\IHKA_rat_RF\modelPCP4.mat';
modelPath{3,1} = 'R:\McKenzieLab\Analysis\SeizureForecasting\IHKA_rat_RF\modelPCP4.mat';
modelPath{4,1} = 'R:\McKenzieLab\Analysis\SeizureForecasting\IHKA_rat_RF\modelPCP4.mat';
inputDur = 4000; % Amount of time to input into classifier in ms
stimClass = 4; % Label to trigger stim
                ...
                if allClear == 0
                    ...
                    for i = SB:size(currentBlock,1)
                        ...
                        for ii = 1:currentTimes(i,1)
                            ...
                            [amplifierData,timeStamps,offset] = byte2doubleConv(waveformArray,numChan,blocksToRead_hs1,sF,dSF,false);
                            ...
                            if usePrediction == 1
                                for i3 = 1:length(predictModel)
                                    spmdSend(timeWindow(:,1:inputDur,i3),predictModel(1,i3),ampData);
                                end
                            end
                           ...
                           % Wait till 0.75s on Intan has passed
                            ...
                            [amplifierData,timeStamps,~] = byte2doubleConv(waveformArray,numChan,blocksToRead_hs2(blockCycle,1),sF,dSF,false);
                            numSample = size(amplifierData,2);
                    ...
                    if spmdProbe(intanIO,ampData) == 1
                        ...
                        if ~any(any(abs(data) > model.ops.art_thres))
                            for i = 1:nChannel
                                ...
                                wave = abs(sm_wavelet(data(i,:)',dSF,freq))';
                                ...
                                for ii = 1:nFreq
                                    feat((ii-1)*nChannel+i) = meanPwr(ii);
                                end
                            end
