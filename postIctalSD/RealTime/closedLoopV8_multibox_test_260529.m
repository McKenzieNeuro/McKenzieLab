...
stimParam(1).config(3).subject = 'PTP5.2'; % Subject in box 3
...
% Prediction:
usePrediction = 1;
modelPath{1,1} = 'R:\ASommer\Matlab\Models\model_pilo_bilateral_27FEB26_used_14MAY26.mat';
modelPath{2,1} = 'R:\ASommer\Matlab\Models\model_pilo_bilateral_27FEB26_used_14MAY26.mat';
modelPath{3,1} = 'R:\ASommer\Matlab\Models\model_pilo_bilateral_27FEB26_used_14MAY26.mat';
modelPath{4,1} = 'R:\ASommer\Matlab\Models\model_pilo_bilateral_27FEB26_used_14MAY26.mat';
...
activeChan(1,1:2,3) = [{'c-000'},{'CA1(L)-3mm'}];
activeChan(2,1:2,3) = [{'c-001'},{'CA1(L)-2mm'}];
activeChan(3,1:2,3) = [{'c-002'},{'CA1(L)-1mm'}];
activeChan(4,1:2,3) = [{'c-003'},{'M2(L)'}];
activeChan(5,1:2,3) = [{'c-004'},{'CA1(R)-3mm'}];
activeChan(6,1:2,3) = [{'c-005'},{'CA1(R)-2mm'}];
activeChan(7,1:2,3) = [{'c-006'},{'CA1(R)-1mm'}];
activeChan(8,1:2,3) = [{'c-007'},{'M2(R)'}];
                ...
                if allClear == 0
                    ...
                    for i = SB:size(currentBlock,1)
                        ...
                        for ii = 1:currentTimes(i,1)
                            ...
                            [amplifierData,timeStamps,offset] = byte2doubleConv(waveformArray,numChan,formatBytesPerBlock,blocksToRead_hs1,sF,dSF,false);
                            disp('Read Array')
                           ...
                           % Wait till 0.75s on Intan has passed
                            ...
                            [amplifierData,timeStamps,~] = byte2doubleConv(waveformArray,numChan,formatBytesPerBlock,blocksToRead_hs2(blockCycle,1),sF,dSF,false);
...
function [amplifierData,timeStamps,offset] = byte2doubleConv(waveformArray,numChan,formatBytesPerBlock,blocksToRead,sampleFreq,downsampleFreq,useConv)
    ...
    if offset == 0    
        ...
        if useConv == 0
            ...
            DStimeStamps = [];
            DStimeStamps = lowpass(timeStampsOut,450,sampleFreq,ImpulseResponse='iir');
            DStimeStamps = downsample(DStimeStamps,sampleFreq/downsampleFreq,0);
            timeStamps=DStimeStamps;
        elseif useConv == 1
            waveformArray = convLPFiltFix(waveformArray',downsampleFreq/2,sampleFreq,[],sampleFreq/downsampleFreq)';
            % Separate timestamps
            timeStamps = waveformArray(1:4,:);
            waveformArray(1:4,:) = [];
        end
        ...
        if useConv==0
            ...
            for channel = 1:size(amplifierData, 1)
                ...
                filtData = lowpass(data,450,sampleFreq,ImpulseResponse='iir');
    ...
    function out=convLPFiltFix(data,cutOff,sampleFreq,filtLength,downSampleRatio)
        % data             = input data, processed column-wise
        % cutOff           = lowpass filter cutoff frequency, must not be > sampleFreq/2
        % sampleFreq       = sampling frequency of input data
        % filtLength       = filter length, [] uses default (must be odd)
        % downSampleRatio  = ratio to downsample filtered data, [] to not downsample
    
        % Normalize cutoff to Nyquist
        cutOff = cutOff / (sampleFreq / 2);
    
        % Default filter length (must be odd for symmetric kernel)
        if isempty(filtLength)
            filtLength = 1025;
        end
    
        % Force odd length so kernel is symmetric around zero
        if mod(filtLength, 2) == 0
            filtLength = filtLength + 1;
        end
    
        % Build windowed sinc kernel
        n = -floor(filtLength/2) : floor(filtLength/2);
        B = sinc(cutOff * n) .* cutOff;
    
        % Normalize kernel to unit DC gain
        B = B / sum(B);
    
        padLen = (filtLength - 1) / 2;   % exact integer since filtLength is odd
        ...
        for i3 = 1:size(data, 2)
            ...
            % Replicate-pad edges to suppress boundary transients
            colPadded = [repmat(col(1), padLen, 1); col; repmat(col(end), padLen, 1)];
    
            K = 2^nextpow2(numel(colPadded) + filtLength - 1);
    
            % FFT-based convolution
            c = real(ifft(fft(colPadded, K) .* fft(B, K)));
    
            % Single-step extraction:
            %   +padLen to compensate for filter group delay
            %   +padLen to skip the replicated start padding
            startIdx = padLen + 1;
            data(:, i3) = c(startIdx : startIdx + nCol - 1);
        end
