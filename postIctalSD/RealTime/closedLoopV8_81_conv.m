% Requires: Computer Vision Toolbox, Mapping Toolbox, Webcam Addon, Machine
% Learning Toolbox

% Lines commented out for testing: 840-867, 1089-1097, 1132-1145
...
stimParam(1).config(1).subject = 'EDS_5.1_L'; % Subject in box 1
stimParam(1).config(2).subject = 'EDS_5.1_R'; % Subject in box 2
...
% Prediction:
usePrediction = 1;
modelPath{1,1} = 'R:\Analysis\McKenzieLab\postIctalSD\Refactor\Models\model_pilo_bilateral_27FEB26_sk.mat';
modelPath{2,1} = 'R:\Analysis\McKenzieLab\postIctalSD\Refactor\Models\model_pilo_bilateral_27FEB26_sk.mat';
modelPath{3,1} = 'R:\Analysis\McKenzieLab\postIctalSD\Refactor\Models\model_pilo_bilateral_27FEB26_sk.mat';
modelPath{4,1} = 'R:\Analysis\McKenzieLab\postIctalSD\Refactor\Models\model_pilo_bilateral_27FEB26_sk.mat';
...
lengthTW_Filt = 5*dSF; % Amount of data held for filter calculations in ms.
lengthTW_Mean = 1*dSF; % Amount of data held for averaging in ms. Must be <= lengthTW_Filt

% Active channels for RHS
activeChan(1,1:2,1) = [{'a-000'},{'CA1(L)'}];
activeChan(2,1:2,1) = [{'a-001'},{'CA1(L)'}];
...
activeChan(1,1:2,2) = [{'b-008'},{'CA1(R1)'}];
activeChan(2,1:2,2) = [{'b-009'},{'CA1(R2)'}];
activeChan(3,1:2,2) = [{'b-010'},{'CA1(R3)'}];
activeChan(4,1:2,2) = [{'b-011'},{'CA1(R4)'}];
activeChan(5,1:2,2) = [{'b-012'},{'CA1(R5)'}];
activeChan(6,1:2,2) = [{'b-013'},{'CA1(R6)'}];
activeChan(7,1:2,2) = [{'b-014'},{'CA1(R7)'}];
activeChan(8,1:2,2) = [{'b-015'},{'CA1(R8)'}];
                    ...
                    for i = SB:size(currentBlock,1)
                        ...
                        for ii = 1:currentTimes(i,1)
                          ...
                          % disp('here50')
                            ...
                            % disp('here70')
                            [amplifierData,timeStamps,offset] = byte2doubleConv(waveformArray,numChanStream,blocksToRead_hs1,sF,dSF,true);
                            % disp('here80')
                            ...
                            if usePrediction == 1
                                ...
                                %fix for multi subject
                                iCore = 1;
                                i3 = 1;
                                % disp('here100')
                                spmdSend({i3,timeWindow(:,1:inputDur)},predictModel(1,iCore),ampData);
                                  % disp('here110')
                            ...
                            %change for multi rat
                            for i3 = 1%1:size(seshStim,1)
                                if activeBox(1,i3) == 1
                            ...
                            intanRead_RHS.NumBytesAvailable
                            % Update data plots
                            if showData == 1
                           ...
                           % Wait till 0.75s on Intan has passed
                            ...
                            [amplifierData,timeStamps,~] = byte2doubleConv(waveformArray,numChanStream,blocksToRead_hs2(blockCycle,1),sF,dSF,true);
                            numSample = size(amplifierData,2);
                ...
                while 1                    
                    if spmdProbe(intanIO,ampData) == 1
                        ...
                        if ~isempty(data)
                            ...
                            disp(size(data))
                            data = data/0.195;
                            data = data(modelOPs.ch_subj,:);

                            if check < 20
                                s = struct("savedata",data);
                                save(['datacheck_shift',num2str(check),'.mat'],"-fromstruct",s);
                                check = check+1;
                            end
                            ...
                            if ~any(any(abs(data) > inf))
                                feat = modelOPs.features(data',[],modelOPs);
...
function [amplifierData,timeStamps,offset] = byte2doubleConv(waveformArray,numChanStream,blocksToRead,sampleFreq,downsampleFreq,useConv)
    ...
    if offset == 0    
        ...
        % Pre-allocate memory
        amplifierData = 32768*ones(numChanStream,size(waveformArray,2));
        
        % Decode each byte pair into a single measurement
        iAmp = 1;
        for i = 1:2:numChanStream*2
