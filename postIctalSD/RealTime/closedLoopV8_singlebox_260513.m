...
%predictModel_range = 4:11;
%predictModel = predictModel_range(1:sum(activeBox)+1);
...
% debug_cores=[intanIO,thetaCalc,deltaCalc,predictModel,dataSaver,camReader,camSaver,frameLabeler,setup,DBGplotData];
                    ...
                    for i = SB:size(currentBlock,1)
                        ...
                        for ii = 1:currentTimes(i,1)
                           ...
                           % Wait till 0.75s on Intan has passed
                            ...
                            if debug(intanIO); disp(amplifierData); end
                            ...
                            timeWindow = circshift(timeWindow,-numSample,2);
                            if debug(intanIO); disp('Line 1506'); disp('time window'); disp(timeWindow); end

                            timeWindow(:,end-numSample+1:end, :) = readWindow;
                            if debug(intanIO); disp('Line 1509'); disp('time window'); disp(timeWindow); end
                    ...
                    if spmdProbe(intanIO,ampData) == 1
                        ...
                        if ~isempty(data)
                            ...
                            if check < 20
                                ...
                                save(['datacheck_shift',num2str(check),'_core',num2str(boxID),'.mat'],"-fromstruct",s);
                                check = check+1;
                            end
