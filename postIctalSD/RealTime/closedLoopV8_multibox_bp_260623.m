...
%   260529: 
%       Change filtering to lowpass 450 Hz then Downsample
%       Add timestamp downsampling
%       
%       Method:      
%           Add DBGwaveform to debug options, saves out waveformData and
%           amplifierData when read
%   260604
%       Use filtfilt for filtering, remove all Conv filters, add corrected timestamp
%       saving
%
%   260610
%       Add selection of model chanels 
%       Add save sessiondata for each subject
%
%   260623
%       Change low-pass filter to band-pass (0.1 Hz to 450 Hz) at downsample
%       Use model from 061826 trained on band-passed data
%
%   260625
%       Changed box to read idxRead(count,2) = i4;
%
%
...
stimParam(1).config(1).subject = ''; % Subject in box 1
stimParam(1).config(2).subject = ''; % Subject in box 2
stimParam(1).config(3).subject = 'PTP 5.2'; % Subject in box 3
stimParam(1).config(4).subject = 'PTP 6.1'; % Subject in box 4
...
setup=frameLabeler+1;
DBGData=setup+1;
DBGplotData=DBGData+1;
DBGdiary=DBGplotData+1;
DBGwaveform=DBGdiary+1;
...
% To print debug statements for a given step or core, put its core number
% in to the debug_cores array like so:
% debug_cores=[intanIO,thetaCalc,deltaCalc,predictModel,dataSaver,camReader,camSaver,frameLabeler,setup,DBGplotData, DBGdiary];
% setup, to debug steps occurring outside of the parallel loop, should be 0

debug_cores=[intanIO,predictModel,dataSaver,frameLabeler,DBGplotData,DBGdiary,DBGwaveform];
debug=zeros(DBGdiary,1);
debug(debug_cores)=1;
...
% Intan TCP:
useRHS = 0;
intanIP_RHS = '127.0.0.0'; % IP for RHS.
...
saveStim = 0; % Save downsampled stim timeSpamps, 1 = yes, 0 = no
showData = 1; % Plot theta/dela data.
lengthData = 60; % seconds of data to store for plot.
saveTD = 1; % Save theta/delta to .dat.

saveAny=any([saveDS,saveTS,saveTD,saveStim]);
...
stimParam(2).config(1).chan = [nan,nan];
stimParam(2).config(2).chan = [nan,nan];
stimParam(2).config(3).chan = [nan,nan];
stimParam(2).config(4).chan = [nan,nan];

% Amplitudes in uA.
stimParam(2).config(1).amp = 0;
stimParam(2).config(2).amp = 0;
stimParam(2).config(3).amp = 0;
stimParam(2).config(4).amp = 0;
...
stimParam(2).config(4).duration = 200;
...
for i = 1:4
    ...
    pulsNum(i,1) = round(stimFreq(i,1)*stimDur(i,1));
    period(i,1) = 1000/stimFreq(i,1);
    if pulsNum(i,1) > 256

        pulsNum(i,1) = round(stimFreq(i,1));
        pulsNum(i,2) = stimDur(i,1);
        period(i,1) = 1000/stimFreq(i,1);
        while pulsNum(i,1)*period(i,1) > 950
            pulsNum(i,1) = pulsNum(i,1)-1;
        end
    end
end

% Number stim train of pulses
% stimParam(2).config(1).pulseNum = pulsNum(1,1);
stimParam(2).config(1).pulseNum = 4;
stimParam(2).config(2).pulseNum = 4;
stimParam(2).config(3).pulseNum = 4; 
stimParam(2).config(4).pulseNum = 4; 

% Train period in mS
% stimParam(2).config(1).period = period(1,1);
stimParam(2).config(1).period = 10;
stimParam(2).config(2).period = 10;
stimParam(2).config(3).period = 10;
stimParam(2).config(4).period = 10;

% Inter stim intervals in seconds, randomly selected.
ISI{1,1} = 30:35; % Open loop
...
% Channel, [nan,nan] = unused, [c1,nan] = monopolar, [c1,c2] = bipolar.
stimParam(3).config(1).chan = [nan,nan];
stimParam(3).config(2).chan = [nan,nan];
stimParam(3).config(3).chan = [nan,nan];
stimParam(3).config(4).chan = [nan,nan];

% Amplitudes in uA.
stimParam(3).config(1).amp = 0;
...
% Number stim train of pulses
stimParam(3).config(1).pulseNum = 100;
stimParam(3).config(2).pulseNum = 100; 
stimParam(3).config(3).pulseNum = 100; 
stimParam(3).config(4).pulseNum = 100; 
...
tdChan(1,1) = nan; % Port A
tdChan(2,1) = nan; % Port B
% tdChan(3,1) = dataChan{3,1}(2); % Port C
...
% Prediction:
usePrediction = 1;
modelPath{1,1} = 'R:\ASommer\Matlab\Models\model_pilo_bilateral_18JUN26.mat';
modelPath{2,1} = 'R:\ASommer\Matlab\Models\model_pilo_bilateral_18JUN26.mat';
modelPath{3,1} = 'R:\ASommer\Matlab\Models\model_pilo_bilateral_18JUN26.mat';
modelPath{4,1} = 'R:\ASommer\Matlab\Models\model_pilo_bilateral_18JUN26.mat';


% Analysis Settings
runAnalysis = 1;
sF = 20000; % Recording sample frequency
dSF = 1250; % Downsample frequency
use60HzFilt = 0;
ACfiltPath = 'R:\DGregg\MatLab\CNN60Hz.mat';
replaceFile = 1;

inputDur = 4*dSF; % Amount of time to input into classifier in ms

windowSec=5;
lengthTW_Filt = windowSec*dSF; % Amount of data held for filter calculations in ms.
lengthTW_Mean = 1*dSF; % Amount of data held for averaging in ms. Must be <= lengthTW_Filt

stimClass = 2; % Label to trigger stim
numConsec = 5*dSF; % Threshold of stimClass needed to trigger stim

activeChan(1,1:2,1) = [{'a-000'},{'CA1(R)-3mm'}];
activeChan(2,1:2,1) = [{'a-001'},{'CA1(R)-2mm'}];
activeChan(3,1:2,1) = [{'a-002'},{'CA1(R)-1mm'}];
activeChan(4,1:2,1) = [{'a-003'},{'M2(R)'}];
activeChan(5,1:2,1) = [{'a-004'},{'CA1(L)-3mm'}];
activeChan(6,1:2,1) = [{'a-005'},{'CA1(L)-2mm'}];
activeChan(7,1:2,1) = [{'a-006'},{'CA1(L)-1mm'}];
activeChan(8,1:2,1) = [{'a-007'},{'M2(L)'}];

activeChan(1,1:2,2) = [{'b-000'},{'CA1(R)-3mm'}];
activeChan(2,1:2,2) = [{'b-001'},{'CA1(R)-2mm'}];
activeChan(3,1:2,2) = [{'b-002'},{'CA1(R)-1mm'}];
activeChan(4,1:2,2) = [{'b-003'},{'M2(R)'}];
activeChan(5,1:2,2) = [{'b-004'},{'CA1(L)-3mm'}];
activeChan(6,1:2,2) = [{'b-005'},{'CA1(L)-2mm'}];
activeChan(7,1:2,2) = [{'b-006'},{'CA1(L)-1mm'}];
activeChan(8,1:2,2) = [{'b-007'},{'M2(L)'}];

activeChan(1,1:2,3) = [{'c-000'},{'CA1(R)-3mm'}];
activeChan(2,1:2,3) = [{'c-001'},{'CA1(R)-2mm'}];
activeChan(3,1:2,3) = [{'c-002'},{'CA1(R)-1mm'}];
activeChan(4,1:2,3) = [{'c-003'},{'M2(R)'}];
activeChan(5,1:2,3) = [{'c-004'},{'CA1(L)-3mm'}];
activeChan(6,1:2,3) = [{'c-005'},{'CA1(L)-2mm'}];
activeChan(7,1:2,3) = [{'c-006'},{'CA1(L)-1mm'}];
activeChan(8,1:2,3) = [{'c-007'},{'M2(L)'}];

activeChan(1,1:2,4) = [{'d-008'},{'M2(R)1'}];
activeChan(2,1:2,4) = [{'d-009'},{'M2(R)2'}];
activeChan(3,1:2,4) = [{'d-010'},{'CA1(R)-1mm'}];
activeChan(4,1:2,4) = [{'d-011'},{'CA1(R)-2mm'}];
activeChan(5,1:2,4) = [{'d-012'},{'M2(L)1'}];
activeChan(6,1:2,4) = [{'d-013'},{'M2(L)2'}];
activeChan(7,1:2,4) = [{'d-014'},{'CA1(L)-1mm'}];
activeChan(8,1:2,4) = [{'d-015'},{'CA1(L)-2mm'}];
...
for i = 1:size(activeChan,3)
    ...
    if ~isempty(stimParam(1).config(i).subject)
        [chanChoice,~] = listdlg('PromptString',{'Select active channels for';['port ',port{1,i},'.']},...
            'ListString',activeChan(:,1,i),'SelectionMode','multiple','Initialvalue',[dataChan{i,1}]);
...
if labelFrames == 1
    ...
    CNNPathName = 'R:\Analysis\SeizureForecasting\NNforecasting\';
end

for i = 1:size(activeChan,3)
    dataChan(:,3,i) = {0};
    if ~isempty(stimParam(1).config(i).subject)
        [chanChoice,~] = listdlg('PromptString',{'Select data channels for';['port ',port{1,i},'.']},...
            'ListString',activeChan(:,1,i),'SelectionMode','multiple','Initialvalue',[dataChan{i,1}]);
        dataChan(chanChoice,3,i) = {1};
    end
end
...
% save sessiondata structure
ID = 0;
for ii = 1:length(stimParam(1).config)
    if ~isempty(stimParam(1).config(ii).subject)
        sessiondata.subject = stimParam(1).config(ii).subject;
        sessiondata.channelID = ID+1:ID+length(activeChan(:,:,ii)); % this method of stating the channels should take into account empty boxes
        ID = ID+length(activeChan(:,:,ii));
        sessiondata.channel = activeChan(:,2,ii);
        sessiondata.model = modelPath{ii,1};
        sessiondata.scriptName = mfilename;
        sessiondata.program = program;
        sessiondata.schedule = schedule;
        sessiondata.blockDurations = duration;
        sessiondata.modelChans = dataChan{ii,1};
        savename = sprintf('%s\\%s.sessiondata.mat',PathName,sessiondata.subject);
        save(savename,'sessiondata');
    end
end
...
spmd(frameLabeler)
    if (spmdIndex == intanIO) && (begin == 0)
        disp('Init Intan Vars')
        intanSend = 0;
        intanRead_RHS = 0;
        intanRead_RHD = 0;
        Ard = [];
...
for Iter = startIter:size(stimParam(1).config(1).schedule,2)
    ...
    for I = 1:1+usePBL
        ...
        % % Replaced above section with section from closedLoopV9.m
        % currentBlock = cell(1,1,4);
        % currentTimes = nan(1,4);
        % for i = 1:4
        %     if ~isempty(stimParam(1).config(i).subject)
        ...
        %             currentBlock(1:numBlock,1,i) = stimParam(1).config(i).schedule(:,Iter);
        %             for ii = 1:length(stimParam(1).config(i).duration)
        %                 currentTimes(ii,1) = stimParam(1).config(i).duration(ii,1);
        %             end
        %         else
        %             currentBlock{1,1,i} = 'BL';
        %             currentTimes(1,:) = durPBL;
        ...
        if debug(predictModel)==1
            debugFolder=[seshPathName,'\Debugging\'];
            if exist(debugFolder)==0
                mkdir(debugFolder);
            end
            datacheck=0;
            featcheck=0;
        ...
        if debug(DBGdiary)==1
            ...
            Now = clock; %#ok<CLOCK>
            Time = [num2str(Now(1,2)),'-',num2str(Now(1,3)),'-',num2str(Now(1,1)),'(',num2str(Now(1,4)),'.',num2str(Now(1,5)),')'];
            diaryPath = [debugFolder,'diary_',Time,'.txt'];
            diary(diaryPath);
        end
        ...
        spmd(frameLabeler)
            ...
            if spmdIndex == intanIO
                if debug(intanIO)==1
                    ...
                    try
                        disp('spmdIndex: ',num2str(spmdIndex))
                    catch
                        disp('cannot disp spmdIndex')
                    end
                    try
                        disp('intanRead_RHS: ',num2str(intanRead_RHS))
                    catch
                        disp('cannot disp intanRead_RHS')
                    end
                    try
                        disp('intanRead_RHD: ',num2str(intanRead_RHD))
                    catch
                        disp('cannot disp intanRead_RHD')
                    end
                    try
                        disp('intanSend: ',num2str(intanSend))
                    catch
                        disp('cannot disp intanSend')
                    end
                end
                ...
                if debug(intanIO)==1
                    ...
                    send(textOut,'Checking Intan RHS commands')
                end
                ...
                if useRHS == 1
                    t = 0;
                    if ~isa(intanSend,'tcpclient') || ~isvalid(intanSend)
                        while 1
                            try
                                disp('RHS 1')
                        ...
                        flush(intanRead_RHS)
                        send(textOut,'Reusing existing RHS command connection')
                    end
                else
                    send(textOut,'RHS disabled')
                end

                send(textOut,'Checking Intan RHS data output')

                % Connect to RHS data output and test connection
                if useRHS == 1
    
                    t = 0;
                    if ~isa(intanRead_RHS,'tcpclient') || ~isvalid(intanRead_RHS)
                       while 1     
                            try
                        ...
                        flush(intanRead_RHS)
                        send(textOut,'Reusing existing RHS data connection')
                    end
                else
                    send(textOut,'RHS disabled')
                ...
                send(textOut,'Checking Intan RHD commands')
                ...
                if useRHD == 1
                    ...
                    if ~isa(intanSend,'tcpclient') || ~isvalid(intanSend)
                        while 1     
                            try 
                                send(textOut,'Connecting to Intan RHD commands')
                                intanSend = tcpclient(intanIP_RHD,intanPort1_RHD);
                                disp(['valid  ', num2str(isvalid(intanSend))])
                                write(intanSend,uint8('execute RescanPorts;'));
                                send(textOut,'Connected to Intan RHD commands')
                                break
                            catch
                                try                               
                                    send(textOut,'Retrying Connection to Intan RHD commands')
                                    write(intanSend,uint8('execute RescanPorts;'));
                                    send(textOut,'Connected to Intan RHD commands')
                                    break
                                catch
                        ...
                        send(textOut,'Reusing existing RHD command connection')
                    end
                else
                    send(textOut,'RHD disabled')
                end
                send(textOut,'Checking Intan RHD data output')
                ...
                if useRHD == 1
                    ...
                    if ~isa(intanRead_RHS,'tcpclient') || ~isvalid(intanRead_RHS)
                        while 1
                            try
                                send(textOut,'Connecting to Intan RHD data output')
                                intanRead_RHS = tcpclient(intanIP_RHD,intanPort2_RHD);
                                flush(intanRead_RHS)
                                send(textOut,'Connected to Intan RHD data output')
                                break
                            catch
                                try
                                    send(textOut,'Retrying Connection to Intan RHD data output')
                                    flush(intanRead_RHS)
                                    send(textOut,'Connected to Intan RHD data output')
                                    break
                                catch
                        ...
                        send(textOut,'Reusing existing RHD data connection')
                    end
                else
                    send(textOut,'RHD disabled')
                end
                ...
                if ~isa(intanSend,'tcpclient') || ~isvalid(intanSend)
                    disp('Valid loop works')
                end

                % Connect to Arduino and test connection
                t = 0;
                if isempty(Ard) || ~isvalid(Ard)
                    while 1
                        try
                    ...
                    send(textOut,'Reusing existing Arduino connection')
                end
                ...
                if allClear == 0
                    ...
                    if intanSend ~= 0 && useRHS==1
                        ...
                        for i = 1:size(activeChan,3)
                            for ii = 1:size(activeChan,1)
                                 if activeChan{ii,3,i} == 1
                                    write(intanSend,uint8(['set ',activeChan{ii,1,i},'.tcpdataoutputenabled true;']));
                                end
                            end
                        end
                        ...
                        if I == 1                                            
                            for i = 2:size(stimParam,2)
                                if i == 2
                                    setStim(intanSend,1,stimParam(2).config)
                                else
                                    if sameS1S2 == 0
                                        setStim(intanSend,1,stimParam(3).config)
                                    end
                                end
                            end
                        end
                    ...
                    if intanSend ~= 0 && useRHD==1
                        write(intanSend,uint8(['set Filename.Path ',seshPathName,';']));
                        write(intanSend,uint8('set FileFormat OneFilePerSignalType;'));
                        write(intanSend,uint8('set Filename.BaseFilename RHD;'));
                    ...
                    if useRHS==1
                        formatBytesPerBlock=128;
                    else
                        formatBytesPerBlock=128;
                    end
                    waveformBytesPerFrame = 4+2*numChan;
                    waveformBytesPerBlock = formatBytesPerBlock*waveformBytesPerFrame+4;
                    blocksToRead_hs1 = 79;
                    blocksToRead_hs2 = 79;
                    blocksToRead_hs2(2,1) = 79;
                    blocksToRead_hs2(3,1) = 79;
                    ...
                    spmdSend(0,camReader,startSig)
                    disp('near start recording')
                    % Start recording
                    if intanSend ~= 0 && useRHS == 1
                        if recIntan == 0
                            write(intanSend,uint8('set runMode run;'));
                        else
                    ...
                    for i = SB:size(currentBlock,1)
                        ...
                        for ii = 1:currentTimes(i,1)
                            ...
                            if reRun == 1
                                ...
                                pause(0.1)
                                write(intanSend,uint8('execute RescanPorts;'));
                                write(intanSend,uint8(['set Filename.BaseFilename RHS_P',num2str(countP),';']));
                                setStim(intanSend,1,newParams)
                                write(intanSend,uint8('execute uploadstimparameters;'));
                                flush(intanRead_RHS)
                                pause(15)
                                write(intanSend,uint8('set runMode record;'));
                            ...
                            if debug(DBGwaveform) 
                              s = struct("savedata",waveformArray);
                              debugWAData=['waveformarray_shift',num2str(datacheck),'.mat'];
                              save([debugFolder,debugWAData],"-fromstruct",s);
                            end

                            [amplifierData,timeStamps,offset] = byte2doublefiltfilt(waveformArray,numChan,formatBytesPerBlock,blocksToRead_hs1,sF,dSF);
                            disp('Read Array')

                            if debug(DBGwaveform) 
                              s = struct("savedata",amplifierData);
                              debugWAData=['amplifierData_shift',num2str(datacheck),'.mat'];
                              save([debugFolder,debugWAData],"-fromstruct",s);
                            end
                            ...
                            disp('Sorted Amp Data by Subject')

                            % Update time window
                            disp('Circshift Time Window...')

                            timeWindow = circshift(timeWindow,-numSample,2);
                            disp('Updating Time Window...')

                            timeWindow(:,end-numSample+1:end,:) = readWindow;
                            disp('Updated Time Window')

                            % Pull channels for T/D
                            for i3 = 1:size(tdChan,1)
                                if ~isnan(tdChan(i3,1))
                                    tdSend(i3,:) = timeWindow(tdChan(i3,1),:,i3);
                                end
                            end
                            ...
                            if usePrediction == 1
                                ...
                                for i3 = 1:length(activeBox)
                                    ...
                                    if activeBox(1,i3) == 1
                                        spmdSend({i3,timeWindow(idxRead(idxRead(:,1)==i3,2),end-inputDur+1:end,i3)},predictModel(1,iCore),ampData);
                                        iCore = iCore+1;
                                    end
                                end
                            end
                            ...
                            if ~isa(intanSend,'tcpclient') || ~isvalid(intanSend)
                                disp('intanSend TCP connection not present')
                            else
                                disp('still valid')
                            ...
                            disp('Data sent to DataSaver core')
                                    ...
                                    if countISI(i3,seshStim(i3,1,i)) == 0
                                        if stimCount(i3,seshStim(i3,1,i)) < stimNum(1,seshStim(i3,1,i))
                                            if seshStim(i3,2,i) == 1 % O
                                                ...
                                                nextISI(i3,1) = datasample(ISI{1,seshStim(i3,1,i)},1)+stimDur(i3,1);
                                            elseif seshStim(i3,2,i) == 2 % tdC

                                                stim(i3,seshStim(i3,1,i)) = (ToD(i3,1) > 0 && ToD(i3,1) < threshold_tdC);
                                                if stim(i3,seshStim(i3,1,i)) == 1
                                                    stimTrig(i3,seshStim(i3,1,i)) = 1;
                                                    nextISI(i3,1) = datasample(ISI{2,seshStim(i3,1,i)},1)+stimDur(i3,1);
                                                end
                                            elseif seshStim(i3,2,i) == 3 % ptC
                                                if ToD(i3,1) >= threshold_ptC
                                                    ...
                                                    if ptC_Count(i3,seshStim(i3,1,i)) >= sustain_ptC
                                                        stim(i3,seshStim(i3,1,i)) = 1;
                                                        stimTrig(i3,seshStim(i3,1,i)) = 1;
                                                        nextISI(i3,1) = datasample(ISI{3,seshStim(i3,1,i)},1)+stimDur(i3,1);
                                            ...
                                            if stim(i3,seshStim(i3,1,i)) == 1
                                                stimCounter(i3,seshStim(i3,1,i)) = pulsNum(i3,2);
                                            end
                                        end
                                    end
                            ...
                            disp('Update data plots')
                            % Update data plots
                            if showData == 1
                                allData = circshift(allData,-1,2);
                                allData(:,end,:) = dataSec;
                                if debug(DBGplotData); disp(allData); end
                                send(dataOut,{allData,activeBox,WLc,iWL})
                                if debug(DBGplotData); disp('send to dataOut'); end
                           ...
                           % Wait till 0.75s on Intan has passed
                            ...
                            % Send sync signal to camReader
                            spmdSend(0,camReader,timerSig);

                            % Read data and decode
                            waveformArray = read(intanRead_RHS,bytesToRead_hs2(blockCycle,1));
                            [amplifierData,timeStamps,~] = byte2doublefiltfilt(waveformArray,numChan,formatBytesPerBlock,blocksToRead_hs2(blockCycle,1),sF,dSF);
                            if debug(intanIO) && debug(DBGData); disp(amplifierData); end

                            numSample = size(amplifierData,2);
                            
                            disp('Sending to dataSaver Core')
                            % Send to datasaver core if enabled
                            saveCell = cell(1,5);
                            if saveDS == 1
                                saveCell{1,1} = amplifierData;
                            ...
                            if debug(intanIO); disp('Line 1501'); disp(readWindow); end

                            % Update time window

                            timeWindow = circshift(timeWindow,-numSample,2);
                            if debug(intanIO) && debug(DBGData); disp('Line 1506'); disp('time window'); disp(timeWindow); end

                            timeWindow(:,end-numSample+1:end, :) = readWindow;
                            if debug(intanIO) && debug(DBGData); disp('Line 1509'); disp('time window'); disp(timeWindow); end

                            disp(['blockCycle: ',num2str(blockCycle)])
                            disp('206')
                    ...
                    if intanSend ~= 0
                        ...
                        if useRHS==1
                            for i = 2:3
                                setStim(intanSend,0,stimParam(i).config)
                            end
                        end
                        ...
                        for i = 1:size(activeChan,3)
                            for ii = 1:size(activeChan,1)
                                 if activeChan{ii,3,i} == 1
                                    write(intanSend,uint8(['set ',activeChan{ii,1,i},'.tcpdataoutputenabled false;']));
                                end
                            end
                        end
                    ...
                    try
                        if spmdProbe(intanIO,ampData) == 1
                            ...
                            try
                                ...
                                disp('thetaCalc: failed to get data')
                                % Do nothing
                            end
                            ...
                            thetaMean = mean(theta(end-lengthTW_Mean:end,:))';
                    ...
                    try
                        if spmdProbe(intanIO,ampData) == 1
                            ...
                            try
                                ...
                                disp('deltaCalc: failed to get data')
                                % Do nothing
                            end
            ...
            elseif any(spmdIndex == predictModel)
                ...
                if usePrediction == 1
                    ...
                    nFreq = length(freq);
                    feat = nan(1,nFreq*nChannel+(2*nChannel));
                ...
                while 1                    
                    if spmdProbe(intanIO,ampData) == 1
                        disp('Entering predictModel')

                        data = spmdReceive(intanIO,ampData);                        
                        if ~isempty(data)
                            if debug(predictModel); disp('getting boxNum'); end

                            boxNum = data{1,1};
                            inputdata = data{1,2};
                            inputdata = inputdata(modelOPs.ch_subj,:);
                            % inputdata = inputdata/0.195;

                            if debug(predictModel)
                                if datacheck < 20
                                    s = struct("savedata",inputdata);
                                    debugFileData=['datacheck_shift',num2str(datacheck),'_core',num2str(boxID),'.mat'];
                                    save([debugFolder,debugFileData],"-fromstruct",s);
                                    datacheck = datacheck+1;
                                end
                            end

                            if debug(predictModel); disp(size(inputdata)); end
                        
                            % Get Feature Extraction function from model alias 
                            %if ~any(any(abs(data) > model.ops.art_thres))
                            if ~any(any(abs(inputdata) > inf))
                                
                                
                                if debug(predictModel); disp('generating feats'); end
                                feat = modelOPs.features(inputdata',[],modelOPs);

                                if debug(predictModel)
                                    if featcheck < 20
                                        s = struct("savedata",feat);
                                        debugFileFeats=['datacheck_feats',num2str(featcheck),'_core',num2str(boxID),'.mat'];
                                        save([debugFolder,debugFileFeats],"-fromstruct",s);
                                        featcheck = featcheck+1;
                                    end
                                end

                                if debug(predictModel); disp('generated feats'); end
                                %run through model
                                [label,conf] = predict(model.rusTree,feat);
                                
                                if debug(predictModel); disp('prediction made'); end
                                
                                disp(conf)
                                spmdSend([boxNum,label],intanIO,modelOut)
                                if debug(predictModel); disp('prediction sent'); end
                            else
                                if debug(predictModel); disp('error, sending empty amp data'); end
                                spmdSend([],intanIO,ampData)
                                if debug(predictModel); disp('error, sent empty amp data'); end
                                spmdSend([boxNum,0],intanIO,modelOut)
                                if debug(predictModel); disp('error, sent no labels'); end
            ...
            elseif spmdIndex == dataSaver
                ...
                comIssue = 0;
                if debug(dataSaver)==1; disp('dataSaver: opening files'); end
                try
                    if saveDS == 1
                    ...
                    if saveTS == 1
                        fID_ts = fopen([seshPathName,'\timestamps_2Ch_',num2str(dSF/1000),'KHz_uint8.dat'],'w');
                     
                    end
                    if saveTD == 1
                        fID_ToD = fopen([seshPathName,'\ToD_2Ch_',num2str(dSF/1000),'_uint8.dat'],'w');
                        
                    end
                    if usePrediction == 1
                        fID_labels = fopen([seshPathName,'\Labels_2Ch_',num2str(dSF/1000),'_uint8.dat'],'w');
                    ...
                    comIssue = 1;
                    if debug(dataSaver)==1; disp('dataSaver: comIssue in file opening'); end

                end
                if debug(dataSaver)==1; disp('dataSaver: sending to intanIO'); end
                spmdSend(comIssue,intanIO,coreCom)
                if debug(dataSaver)==1; disp('dataSaver: sent to intanIO'); end
                comIssue = spmdReceive(intanIO,coreCom);
                if comIssue == 0
                    if debug(dataSaver)==1; disp('dataSaver: no comIssue'); end
                    ...
                    while 1
                        if spmdProbe(intanIO,ampData) == 1
                            ...
                            try
                                ...
                                if debug(dataSaver)==1; disp('dataSaver: error recieving saveCell'); end
                            end

                            if ~isempty(saveCell)
                                if saveDS == 1
                                   fwrite(fID_amp,saveCell{1,1},'int16');
                                   if debug(dataSaver)==1; disp('dataSaver: saveDS complete'); end

                                end
                                if saveTS == 1
                                   fwrite(fID_ts,saveCell{1,2},'uint8');
                                   if debug(dataSaver)==1; disp('dataSaver: saveTS complete'); end

                                end
                                if saveTD == 1
                                   fwrite(fID_ToD,saveCell{1,3}*25,'uint8');
                                   if debug(dataSaver)==1; disp('dataSaver: saveTD complete'); end

                                end
                                if usePrediction == 1
                                    fwrite(fID_labels,saveCell{1,4},'uint8');
                                    if debug(dataSaver)==1; disp('dataSaver: savePrediction complete'); end

                                end
                                if saveStim == 1
                                    fwrite(fID_stim,saveCell{1,5},'uint8');
                                    if debug(dataSaver)==1; disp('dataSaver: savestim complete'); end

                                end
                            else
                                if debug(dataSaver)==1; disp('dataSaver: error in saveCell loop'); disp(saveCell); disp('saveCell'); end
            ...
            elseif spmdIndex == camReader
                ...
                spmdSend(comIssue,intanIO,coreCom)
                comIssue = spmdReceive(intanIO,coreCom);                
                if debug(camReader)==1; disp('camReader connected'); end
                ...
                if comIssue == 0
                    ...
                    if showVideo == 1                        
                        ...
                        if debug(camReader)==1; disp('video sent'); end
                    ...
                    while 1
                        if spmdProbe(intanIO,timerSig) == 1
                            ...
                            % if debug(camReader)==1; disp('camreader: timer recieved'); end
                            ...
                            if ~isempty(go)
                                while spmdProbe(intanIO,timerSig) == 0
                                    ...
                                    % if debug(camReader)==1; disp('camreader: videoData sent'); end
            ...
            elseif spmdIndex == camSaver    
                if debug(camSaver)==1; disp('camSaver: init'); end
                ...
                if RecVid == 1
                    try
                        ...
                        if debug(camSaver)==1; disp('camSaver: video file opened'); end
                        ...
                        if labelFrames == 1
                            ...
                            for ii = 1:numCam    
                                ...
                                writerObj_Pose{ii,1} = VideoWriter([seshPathName,'\',VidFileName{ii,1}],'MPEG-4'); %#ok<TNMLP>
                                writerObj_Pose{ii,1}.FrameRate = fps;
                                open(writerObj_Pose{ii,1});
                            end
                            if debug(camSaver)==1; disp('camSaver: pose video file opened'); end
                ...
                if comIssue == 0
                    if debug(camSaver)==1; disp('camSaver: no comIssue'); end
                    ...
                    while 1
                        ...
                        if debug(camSaver)==1; disp('entering RecVid loop'); end
                       ...
                       if RecVid == 1
                            ...
                            if debug(camSaver)==1; disp('camSaver: video segment written'); end
            ...
            elseif spmdIndex == frameLabeler                
                if debug(frameLabeler)==1; disp('frameLabeler: init'); end
                ...
                if comIssue == 0
                    if debug(frameLabeler)==1; disp('frameLabeler: no comIssue'); end
                    ...
                    while 1
                        ...
                        try
                            ...
                            if debug(frameLabeler)==1; disp('frameLabeler: no frames recieved'); end
                        ...
                        if ~isempty(frames)
                            ...
                            if debug(frameLabeler)==1; disp('frameLabeler: loading netCNN'); end

                            YPred = predict(netCNN,framesR);
                            
                            if debug(frameLabeler)==1; disp('frameLabeler: loaded netCNN'); end
                            ...
                            if debug(frameLabeler)==1; disp('frameLabeler: sent frames'); end
        ...
        if runAnalysis == 1
            ...
            for i = 1:numFolders
                ...
                elseif contains(listing(i).name,'Debugging') == 1
                    recInfo(i).system = 'RHD';
                end
...
function makeBox(input)
    ...
    if ~isempty(input)
        if input ~= 1
            ...
            qSend = input;
            closeBox = errordlg('Press ok to stop.','Stop Closed Loop','non-modal');
...
function [amplifierData,timeStamps,offset] = byte2doublefiltfilt(waveformArray,numChan,formatBytesPerBlock,blocksToRead,sampleFreq,downsampleFreq)
    
    % Inputs:
       % waveformArray: TCP stream from Intan TCP connection
       %    size: (1, 1:bytesToRead_hs2(blockCycle,1)) samples
       %    unit: pairs of bytes
       %
       % numChan: total number of channels to be streamed over TCP
       %    unit: integer
       %
       % formatBytesPerBlock: Bytes sent per recording block, always 128
       %  whether Intan RHS or RHD format
       %    unit: integer
       %
       % blockstoRead: changes based on BlockCycle, unknown how this was
       % determined to be 78 for all blocks < 5
       %    unit: integer
       %
       % sampleFreq: sampling frequency of incoming data
       %    unit: Hz
       %
       % downsampleFreq: output sampling frequency desired
       %    unit: Hz
       %
       % useConv: binary flag, signals use of convolutional filter 
       %    unit: true/false

    filter1 = designfilt('bandpassiir', 'FilterOrder', 2, 'PassbandFrequency1', 0.1, 'PassbandFrequency2', 450, 'SampleRate', sampleFreq);
    ...
    if offset == 0    
        ...
        waveformArray = reshape(waveformArray,[size(waveformArray,1)/formatBytesPerBlock,blocksToRead*formatBytesPerBlock]);
        ...
        timeStampsOut = [];
        for ii = 1:size(timeStamps,2)
            timeStampsOut(ii)=typecast(uint8(timeStamps(:,ii)),'uint32');
        end

        DStimeStamps = [];
        DStimeStamps = filtfilt(filter1, timeStampsOut);
        DStimeStamps = downsample(DStimeStamps,sampleFreq/downsampleFreq,0);
        timeStamps=DStimeStamps;
        ...
        % Removed uV conversion to match data loaded from 
        % Intan amplifier .dat when loaded with LoadBinary
        amplifierData = amplifierData-32768;
        

        % Downsample data using filtfilt
        DSamplifierData = [];
        for channel = 1:size(amplifierData, 1)
            data = amplifierData(channel, :);
            filtData = filtfilt(filter1,data);
            filtData = downsample(filtData,sampleFreq/downsampleFreq,0);
            DSamplifierData = [DSamplifierData; filtData];
        end
        amplifierData = DSamplifierData;
...
function plotFrame(input)   
    ...
    if videoOut.QueueLength < 10
        if isempty(vidFig) == 1 || ishandle(vidFig) == 0
            vidFig = figure('Name','Closed Loop Video','NumberTitle','off');
            vidAx = cell(1,numCam);
