...
StartTime = [6,10]; % Time to begin recording in 24hr time, format = [hr,min]
...
%       ipC = intelligent trigger (coming soon)
% SX = Stim sham
% OD = Optional delay
...
for i = 1:4
    ...
    elseif program == 3 % Closed loop

        schedule = [{'BL'    },{'BL'},{'BL'    },{'BL'},{'BL'    },{'BL'};...
                    {'S1_ipC'},{'SX'},{'S1_ipC'},{'SX'},{'S1_ipC'},{'SX'};...
                    {'BL'    },{'BL'},{'BL'    },{'BL'},{'BL'    },{'BL'}];

        duration = [60;3600;3600];

    elseif program == 4 % Induction peak
    ...
    elseif program == 5 % Induction decay
    ...
    stimParam(1).config(1).duration = duration;
end
...
stimParam(2).config(1).chan = [6,nan];
...
pulsNum = round(stimFreq*stimDur);
period = 1000/stimFreq;

% Number stim train of pulses
stimParam(2).config(1).pulseNum = pulsNum;
stimParam(2).config(2).pulseNum = 4; 
stimParam(2).config(3).pulseNum = 4; 
stimParam(2).config(4).pulseNum = 4; 

% Train period in mS
stimParam(2).config(1).period = period;
...
ISI{5,1} = stimDur+10; % Intelligent predictor closed loop
...
% Amplitudes in uA.
stimParam(3).config(1).amp = 0;%100;
stimParam(3).config(2).amp = 0;%400;
stimParam(3).config(3).amp = 0;%200;
stimParam(3).config(4).amp = 0;%250;
...
lengthTW_Mean = 5000; % Amount of data held for averaging in ms. Must be <= lengthTW_Filt
showData = 1; % Plot theta/dela data.
lengthData = 60; % seconds of data to store for plot.
saveData = 1; % Save theda/delta to .dat.
syncIntan = 1; % Sync system to RHS, RHS must be enabled with a sample rate of 20000Hz and video fps of 25
...
% Prediction:
usePrediction = 1;
modelPath{1,1} = 'R:\Analysis\SeizureForecasting\IHKA_rat_RF\classificationPCP.mat';
modelPath{2,1} = 'R:\Analysis\SeizureForecasting\IHKA_rat_RF\classificationPCP.mat';
modelPath{3,1} = 'R:\Analysis\SeizureForecasting\IHKA_rat_RF\classificationPCP.mat';
modelPath{4,1} = 'R:\Analysis\SeizureForecasting\IHKA_rat_RF\classificationPCP.mat';
inputDur = 2000; % Amount of time to input into classifier in ms
stimClass = 5;
...
for Iter = startIter:size(stimParam(1).config(1).schedule,2)
    ...
    for I = 1:1+usePBL
        ...
        seshStim = zeros(size(currentBlock,3),2,size(currentBlock,1));
        ...
        spmd(frameLabeler)
            ...
            if spmdIndex == intanIO
                ...
                if allClear == 0
                    ...
                    if showData == 1
                        send(dataOut,{allData,WLc,iWL})
                    end
    
                    if usePrediction == 1
                        send(labelOut,{allLabels,WLc,iWL})
                    end
                    ...
                    for i = SB:size(currentBlock,1)
                        for ii = 1:currentTimes(i,1)                    
                            ...
                            if reRun == 1
                                ...
                                setStim(intanSend_RHS,1,newParams)
                                write(intanSend_RHS,uint8('execute uploadstimparameters;'));
                                flush(intanRead_RHS)
                                pause(15)
                                write(intanSend_RHS,uint8('set runMode record;'));
                            ...
                            [amplifierData,offset] = byte2doubleConv(waveformArray,numChan,blocksToRead,sF,dSF);
                            ...
                            readWindow = zeros(size(activeChan,1),size(amplifierData,2),size(activeChan,3));
                            iChan = 1;
                            for i3 = 1:size(activeChan,3)
                                for i4 = 1:size(activeChan,1)
                                    if any(dataChan{i3,1} == i4-1)
                            ...
                            if usePrediction == 1
                                for i3 = 1:length(predictModel)
                                    TW = timeWindow(:,end-inputDur+1:end,i3);
                                    spmdSend(timeWindow(:,end-inputDur+1:end,i3),predictModel(1,i3),ampData);
                                end
                            end
                                        ...
                                        if stimCount(i3,seshStim(i3,1,i)) < stimNum(1,seshStim(i3,1,i))
                                            ...
                                            elseif seshStim(i3,2,i) == 3 % ptC
                                                if ToD(i3,1) >= threshold_ptC
                                                    ...
                                                    if ptC_Count(i3,seshStim(i3,1,i)) >= sustain_ptC
                                                        stim(i3,seshStim(i3,1,i)) = 1;
                                                        stimTrig(i3,seshStim(i3,1,i)) = 1;
                                                        nextISI(i3,1) = datasample(ISI{3,seshStim(i3,1,i)},1);
                                            ...
                                            elseif seshStim(i3,2,i) == 4 % dtC
                                                ...
                                                elseif ToD(i3,1) > 0 && ToD(i3,1) < threshold_dtC && pastPeak(i3,seshStim(i3,1,i)) == 1
                                                    ...
                                                    if dtC_Count(i3,seshStim(i3,1,i)) >= sustain_dtC                            
                                                        stim(i3,seshStim(i3,1,i)) = 1;
                                                        stimTrig(i3,seshStim(i3,1,i)) = 1;
                                                        nextISI(i3,1) = datasample(ISI{4,seshStim(i3,1,i)},1);
                                            ...
                                            elseif seshStim(i3,2,i) == 5 % ipC
                                                if labels(i3,1) == stimClass
                                                    stim(i3,seshStim(i3,1,i)) = 1;
                                                    stimTrig(i3,seshStim(i3,1,i)) = 1;
                                                    nextISI(i3,1) = datasample(ISI{5,seshStim(i3,1,i)},1);
                                                end
                                            end
                                        end
                                    end
                            ...
                            % Update data plots
                            if showData == 1
                                allData(:,end+1,:) = dataSec; %#ok<SAGROW>
                                allData(:,1,:) = [];
                                send(dataOut,{allData,WLc,iWL})
                            end
                ...
                while 1
                    try
                        if spmdProbe(intanIO,ampData) == 1
                            ...
                            theta = mean(theta(end-lengthTW_Mean+1:end,:))';
        
                            spmdSend(theta,intanIO,ampData)
                        else
                            pause(0.001)
                        end
                    catch
                ...
                while 1
                    try
                        if spmdProbe(intanIO,ampData) == 1
                            ...
                            delta = mean(delta(end-lengthTW_Mean+1:end,:))';
        
                            spmdSend(delta,intanIO,ampData)
                ...
                while 1                    
                    if spmdProbe(intanIO,ampData) == 1
                        ...
                        if ~any(any(abs(data) > model.ops.art_thres))
                            ...
                            spmdSend([id,label],intanIO,modelOut)
                        else
            ...
            elseif spmdIndex == dataSaver
                ...
                try
                    if saveDS == 1
                        fID_amp = fopen([seshPathName,'\amplifier_',num2str(numChan),'Ch_',num2str(dSF/1000),'KHz.dat'],'w');
                    end
                    if saveData == 1
                        fID_ToD = fopen([seshPathName,'\ToD_',num2str(length(tdChan)),'Ch_1Hz.dat'],'w');
                    end
                    if usePrediction == 1
                        fID_labels = fopen([seshPathName,'\Labels_',num2str(length(tdChan)),'Ch_1Hz.dat'],'w');
                    end
                catch
                    comIssue = 1;
                end
                    ...
                    while 1
                        if spmdProbe(intanIO,ampData) == 1
                            ...
                            if ~isempty(saveCell)
                                ...
                                if saveData == 1
                                    fwrite(fID_ToD,saveCell{1,2}*100,'int16');
                                end
                                if usePrediction == 1
                                    fwrite(fID_labels,saveCell{1,3}*100,'int16');
                                end
                            else
                                break
                            end
                ...
                if comIssue == 0
                    ...
                    while 1
                        if spmdProbe(intanIO,timerSig) == 1
                            ...
                            t = spmdReceive(intanIO,timerSig);
        ...
        if runAnalysis == 1
            ...
            for i = 1:numFolders
                ...
                numActChan = length(find(cell2mat(activeChan(:,3,:)) == 1));
                chan = cell(numActChan,2);
                count = 1;
                ...
                numSamples = s.bytes/(2*numActChan);   
                
                fileID = fopen([recInfo(i).file,'\stim.dat']);
                fseek(fileID,0,'bof');
                L = round(numSamples/(sF*600));
                stimData = nan(numActChan,L*600*dSF);
            
                if replaceFile == 1
                    fID = fopen([recInfo(i).file,'\stim_',num2str(numActChan),'Ch_',num2str(dSF/1000),'KHz_.dat'],'w');
                end
                ...
                for ii = 1:L
                    ...
                    temp = fread(fileID,[numActChan,sF*600],'int16');
                    temp = downsample(temp',sF/dSF)';
                    if replaceFile == 1
                        fwrite(fID,temp,'int16');
                    end
                ...
                % data = nan(numActChan,L*600*dSF);
                % thetaAmp = nan(numActChan,L*600*dSF);
                % deltaAmp = nan(numActChan,L*600*dSF);
                % 
                % if replaceFile == 1
                %     fID = fopen([recInfo(i).file,'\amplifier_',num2str(numActChan),'Ch_',num2str(dSF/1000),'KHz.dat'],'w');
                %     if use60HzFilt == 1
                %         load(ACfiltPath);
                %         fID2 = fopen([recInfo(i).file,'\amplifier_',num2str(numActChan),'Ch_',num2str(dSF/1000),'KHz_Filt.dat'],'w');
                %     end
                % end
                % 
                % wb = waitbar(0,'Working on amplifier.dat');
                % Count = 1;
                % tempOut = cell(frameLabeler,1);
                % for ii = 1:frameLabeler:L
                % 
                %     nextI = ii+frameLabeler+1;
                %     if nextI > L
                %         numI = nextI-L;
                %     else
                %         numI = frameLabeler;
                ...
                %     end
                % 
                %     for i3 = 1:frameLabeler
                %         try
                %             tempOut{i3,1} = temp{i3};
                %         catch
                %             continue
                %         end
                %     end
                ...
                %     if replaceFile == 1
                %         for i3 = 1:frameLabeler
                %             fwrite(fID,tempOut{i3,1},'int16');
                %             if use60HzFilt == 1
                %                 for i4 = 1:numActChan
                %                     tempOut{i3,1}(i4,:) = predict(CNN60Hz,tempOut(i4,:)/50000,'ExecutionEnvironment','gpu')*50000;
                %                 end
                %                 fwrite(fID2,tempOut{i3,1},'int16');
                %             end
                %         end
                %     end
                % 
                %     for i3 = 1:frameLabeler
                %         data(:,Count:Count+size(tempOut{i3,1},2)-1) = tempOut{i3,1};
                %         thetaAmp(:,Count:Count+size(tempOut{i3,1},2)-1) = abs(hilbert(bandpass(tempOut{i3,1}',[5,12],dSF)'));
                %         deltaAmp(:,Count:Count+size(tempOut{i3,1},2)-1) = abs(hilbert(bandpass(tempOut{i3,1}',[1,4],dSF)'));
                %         Count = Count+size(tempOut{i3,1},2);
                %     end
...
function [amplifierData,offset] = byte2doubleConv(waveformArray,numChan,blocksToRead,sampleFreq,downsampleFreq)
...
function plotData(input)
    ...
    if isempty(datFig) == 1 || ishandle(datFig) == 0
        ...
        HdatAx = cell(size(data,1),size(data,3));
        for i = 1:size(WL,1)
             
            datAx{1,i} = subplot(1,numChan,i,'Parent',datFig);
            hold(datAx{1,i},'on')
            for ii = 1:size(data,1)            
                HdatAx{ii,i} = plot(datAx{1,i},data(ii,:,i));
            end
            hold(datAx{1,i},'off')
            ylim(datAx{1,i},[0,4])
            ylabel(datAx{1,i},'T/D')
            xticks(0:round(size(data,2)/6):size(data,2))
            xticklabels(size(data,2):-(round(size(data,2)/6)):0)
        ...
        for i = 1:size(WL,1)
            for ii = 1:size(data,1)            
                set(HdatAx{ii,i},'YData',data(ii,:,i))
            end
        end
    end
    drawnow
