...
topDirs{1} = 'R:\DGregg\NeuralData\PTP\Recordings';
% topDirs{1} = 'R:\DGregg\NeuralData\PTP';
fils = [];
...
fils1 = fils(kp);
channelID = [5,6]; %% This will pull only the bilateral hippocampal channels

%%


FeatureFileOutput = 'R:\Analysis\SeizureForecasting\IHKA_rat_RF\features_pilo2.mat';
%FeatureFileOutput = 'E:\data\IHKA\features_trans.mat';
load(FeatureFileOutput)
ClassifierFileOutput =  'R:\Analysis\SeizureForecasting\IHKA_rat_RF\model_pilo.mat';
...
for i = 1:length(fils1)
    fout  = [fileparts(fils1{i}) filesep 'auto_sz_classifier_CA1.mat'];
   % if ~exist(fout)
        v = load(fils1{i});
        ...
        for tim = 1:floor(fileDur)-ops.durFeat
            data = LoadBinary(v.sessiondata.lfp_file,'nchannels',v.sessiondata.xml.nChannels,'frequency', ...
                v.sessiondata.xml.lfpSampleRate,'channels',channelID,'duration', ops.durFeat,'start',tim);
