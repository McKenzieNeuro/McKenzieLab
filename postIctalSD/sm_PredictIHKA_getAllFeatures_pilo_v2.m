...
fils1 = fils(kp);
sessionfils = cellfun(@fileparts,fils1,'uni',0);
...
session_fils = fils1(ismember(sessionfils,goodFils)); % keep only files with subject sessiondata (ALEX)
...
for i= 1:size(sessions,1)
    ...
    if ~isempty(ev.description)
        kp_sz_on = contains(ev.description,'Start') & contains(ev.description,'Seizure');
        kp_sz_off =  contains(ev.description,'Stop')& contains(ev.description,'Seizure');
        kp_noise_on = contains(ev.description,'Start') & contains(ev.description,'Noise');
        kp_noise_off =  contains(ev.description,'Stop')& contains(ev.description,'Noise');
...
% for i = nSessions
    ...
    for k = 1:ops.nBins
        ...
        if k ==1
            ...
            sz{k} = getXPctTim([0 fileDur], ops.pct(k),1);
        else
