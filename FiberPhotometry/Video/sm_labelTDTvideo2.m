function sm_labelTDTvideo2(varargin)
% this function uses the TDT library TDTbin2mat to load in neural data and
% video to manually code events in the video and save to a *.mat file
% 
% Optional inputs
% filename = full path of avi file
...
% check for existing .mat files in same directory as .avi file
matName = [];
eventTimes = [];
eventNames = {};
eventFrames = [];
if(exist([dirname 'newContext.mat'],'file') == 2) % NEE data already coded
	disp("newContext.mat found");
	matName = [dirname 'newContext.mat'];
    mat = load(matName);
    eventTimes = cell2mat(mat.data(:,2));
    eventNames = mat.data(:,1);
elseif(exist([dirname 'novelObject.mat'],'file') == 2) % SOR or OLM data already coded
	disp("novelObject.mat found");
	matName = [dirname 'novelObject.mat'];
    mat = load(matName);
    eventTimes = cell2mat(mat.data(:,2));
    eventNames = mat.data(:,1);
else
	disp("no existing data found");
end
% need food reward elif
...
if ~isempty(eventTimes)
    if max(eventTimes) > max(ts_video)
       
        error(['Max jump time ' num2str(max(eventTimes)) ' is longer than video ' num2str(max(ts_video))])
        
    end
    
    [~,eventFrames] = histc(eventTimes,ts_video);
    event_ii = 1; % global variable to event tracking
end
...
if isstr(fname)
    ...
    f = figure('KeyPressFcn',@(x,y) hotkey(x,y),'CloseRequestFcn',@closeButton);
    ...
    if ~isempty(eventTimes)
        uicontrol('Parent',f,'Style','pushbutton','String','Next Event',...
			'Units','pixels','Position',[747 130 100 50],'Units','normalized',...
			'Callback',@skipNext);
    
		uicontrol('Parent',f,'Style','pushbutton','String','Previous Event',...
			'Units','pixels','Position',[747 70 100 50],'Units','normalized',...
			'Callback',@skipPrevious);
			
        currentEvent = eventNames(event_ii);
		objLabel = text(747,200,['Current Event: ' currentEvent]);
    end
    ...
    if ~isempty(eventTimes)
        % load existing data into presses array
        for i = 1:size(eventTimes)
            ID = find(cellfun(@(a) strcmp(a,eventNames(i)),C));
            presses = [presses;eventFrames(i) ID];
            presses = sortrows(presses,1);
        end
        
        % jump to first event in array
        frame = presses(1,1);
        dispframe(frame);
    end
end
    ...
    function dispframe(n,c)
        if(n < NumberOfFrames && n > 0)
            if(~isempty(vid))
                try
                    ...
                    disp("exception occurred! - " + exception.identifier + ": " + exception.message);
                end
            end
        ...
        % check for event at current frame, and update event label
        if ~isempty(presses) 
            if(ismember(num2str(n),arrayfun(@num2str,presses(:,1),'UniformOutput',false)))
                % update event index
                event_ii = find(presses(:,1)==n);
                set(objLabel,'string',['Current Event: ' C(presses(event_ii,2))]);
            else
                set(objLabel,'string','Current Event: -');
            end
        end
    end
	...
	function skipNext(~,~)
			...
			if event_ii < length(presses)
				event_ii = event_ii+1;
			end
			
			frame = presses(event_ii,1);
    ...
    function clockFrame(obj,event)
        ...
        if isempty(ID)
            ...
            if isempty(presses)
                ...
                % sort to put added frame at the correct event index
                presses = sortrows(presses,1);
                ...
                event_ii = 1;
                objLabel = text(747,200,['Current Event: ' C(presses(event_ii,2))]);
            elseif ~ismember(cellstr(num2str(frame)),arrayfun(@num2str,presses(:,1),'UniformOutput',false))
                presses=[presses;frame ID];
                % sort to put added frame at the correct event index
                presses = sortrows(presses,1);
    ...
    function showData(obj,~)
        ...
        if any(presses)
            figData = mltable(fig, tbl, 'CreateTable', columninfo, rowHeight, [linearize(C(presses(:,2))) num2cell(presses(:,1),2)], gFont);
            uiwait(fig)
        end
        ReleaseFocus(f)
