...
kp = cellfun(@any,regexp(fils,'Novel Env')) & (cellfun(@any,regexp(fils,'NE2')) | cellfun(@any,regexp(fils,'NE3')));
...
clear context  sessionDate subj PETH_novCon PETH_rear
for i = 1:length(dirs)
    ...
    if exist('sessiondata.mat')
        ...
        [ix,early,late,ts_PETH_NC] = sm_getIndicesAroundEvent(ts,300,300,fs,length(sessiondata.neural.signal_DFoF));
        ...
        if exist('rear.mat')
            load('rear.mat')
            ts=  cell2mat(data(contains(data(:,1),'start'),2));
            
            [ix,early,late,ts_PETH] = sm_getIndicesAroundEvent(ts,25,25,fs,length(sessiondata.neural.signal_DFoF));
...
NE_homeCage = cell2mat(cellfun(@(a,b) a(b,:),PETH_novCon,homeCage','uni',0)');
NE_otherContext = cell2mat(cellfun(@(a,b) a(b,:),PETH_novCon,other','uni',0)');

%%
%%Figure 1A1
close all
h0 = figure;
plotMeanSEM(ts_PETH_NC(1:100:end),NE_otherContext(:,1:100:end),'k')

hold on
plotMeanSEM(ts_PETH_NC(1:100:end),NE_homeCage(:,1:100:end),'r')
plot([-300 300],[0 0],'k')
plot([0 0],[-.5 2.25],'k')
xlim([-300 300])
ylim([-.5 2.25])
...
filename = 'E:\Dropbox\UNM\Papers\NE_timeConstants\Figures\Figure1A1.eps';

saveas(h0,filename,'epsc')
...
close all
set(0, 'DefaultFigureRenderer', 'painters');
filename = 'E:\Dropbox\UNM\Papers\NE_timeConstants\Figures\FigureS2_rear.eps';
...
for i = 1:8
    ...
    try
        axes(ax(i))
        tmp = cell2mat(PETH_rear(b==i)');
        tmp = tmp(:,1:100:end);
        plotMeanSEM(ts_PETH(1:100:end),tmp,'k')
...
filename = 'E:\Dropbox\UNM\Papers\NE_timeConstants\Figures\FigureS1A.eps';
%Figure S1a
h1=figure;
subplot(1,4,1)
...
%Figure S1b
set(0, 'DefaultFigureRenderer', 'painters');
filename = 'E:\Dropbox\UNM\Papers\NE_timeConstants\Figures\FigureS1B.eps';
h2 = figure;
subplot(1,4,1)
...
filename = 'E:\Dropbox\UNM\Papers\NE_timeConstants\Figures\Figure1A.eps';
h3 = figure;
...
filename = 'E:\Dropbox\UNM\Papers\NE_timeConstants\Figures\FigureS2.eps';
...
for j = 1:8
 ...
 [rr(:,:,j),p(:,:,j)] =    corr( [vel1(kp) acc1(kp) distEdg1(kp) time_rear(kp) timeFrmEntry(kp)],'rows','pairwise');
end

filename = 'E:\Dropbox\UNM\Papers\NE_timeConstants\Figures\FigureS3-NE.eps';

tmp = nanmean(rr,3);
tmp(eye(size(tmp))==1) = nan;
...
for i = 1:5
    for j = 1:5
        [~,p(i,j)] =  ttest(squeeze(rr(i,j,:)));
    end
end

%%

%load('R:\DANEHippocampalResponse\Workspaces\novelContext_doubleExp.mat')
       ...
       % kp_train = true(sum(kp),1);
        ...
        for k  = usesID
            ...
            if any(kp_sav)
                ...
                load('sessiondata.mat')
                if isfield(sessiondata.neural,'pred')
                    sessiondata.neural = rmfield(sessiondata.neural,'pred');
                end
       ...
       % kp_train = true(sum(kp),1);
        ...
        for k  = usesID
            ...
            if any(kp_sav)
                ...
                pred_ses = nan(length(sessiondata.behavior.ts_video),1);
                pred_ses(ismember(sessiondata.behavior.ts_video,ts_sav)) = pred_sav;
                sessiondata.neural.pred.yhat_red = pred_ses;
                save('sessiondata.mat','sessiondata','-v7.3')
            end
...
set(gca,'xtick',1:6,'xticklabel',{'Intercept','velocity','acceleration','distance2dge','time from transfer','time from rear'})
filename = 'E:\Dropbox\UNM\Papers\NE_timeConstants\Figures\Figure1D';
saveas(h5,filename,'epsc')
%%
        ...
        for k  = usesID
            ...
            if any(kp_sav)
                ...
                load('sessiondata.mat')
                 ...
                 pred_ses = sessiondata.neural.pred.yhat_red;
               % pred_ses = nan(length(sessiondata.behavior.ts_video),1);
                pred_ses(ismember(sessiondata.behavior.ts_video,ts_sav)) = pred_sav;
                sessiondata.neural.pred.yhat_red = pred_ses;
            %    save('sessiondata.mat','sessiondata','-v7.3')
...
lme = fitlme(tbl(kp,:),'tau ~ day + (1+day|subj)')
...
taut = [tau;  b_all_HC(:,6)];
day1 = [day; 11*ones(8,1)];
subjt = linearize(repmat(1:8,11,1));
tbl = table(log10(-taut),subjt,double(day1>10),'VariableNames',{'tau','subj','day'});
lme = fitlme(tbl(kp,:),'tau ~ day + (1+day|subj)')
hold on
boxplot((-taut),day1,'notch','off','whisker',inf)
hold on
ylim([.001 .12])
...
b_all_HC =[];MSE_full =[];MSE_red=[];b_full=[];pred_tim_HC=[];
    ...
    for j = 1:8
      ...
      %  IV =[inter x1];
        ...
        %isTime = [false true];
        [~,~,b_all_HC(j,:)] = sm_model_comp_MSE(IV,DV,isTime,kp_train,false);
