...
for i = 1:length(dirs)
    ...
    if exist([dirs{i} filesep 'sessiondata.mat'])
        if exist('arena_edges.mat')
            if exist([dirs{i} filesep 'contextTransitionRevised.mat'])
                ...
                if exist([dirs{i} filesep 'contextTransitionRevised.mat'])
                    load([dirs{i} filesep 'contextTransitionRevised.mat']);
                ...
                for ind = 1:size(data,1)
                    data{ind,3}(:,1) = sessiondata.contextEntry{ind,3}(:,1)
                    data{ind,3}(:,2) = sessiondata.contextEntry{ind,3}(:,2)
                end
