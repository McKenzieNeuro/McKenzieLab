...
dirs = [ ...
  ...
  %  {'R:\DANEHippocampalResponse\NE2h2 (Named NE2m3)\LinearTrack\NE2h2-220623-130352'} ; ... %(done)
    {'R:\DANEHippocampalResponse\NE2h2 (Named NE2m3)\LinearTrack\NE2h2-220630-112150'} ; ...%(done)
    
   % {'R:\McKenzieLab\DANEHippocampalResponse\NE2h4\LinearTrack\NE2h4-220622-092231'} ; ...
   % {'R:\DANEHippocampalResponse\NE2h4\LinearTrack\NE2h4-220623-112508'} ; ...%(done)
    {'R:\DANEHippocampalResponse\NE2h4\LinearTrack\NE2h4-220630-125426'} ; ...%(done)
    
 
    
    {'R:\DANEHippocampalResponse\NE2h6\Linear Track\NE2h6-220720-090530'} ; ...%(done)
 %   {'R:\DANEHippocampalResponse\NE2h6\Linear Track\NE2h6-220721-074443'} ; ...%(done)
    
    {'R:\DANEHippocampalResponse\NE2h7\Linear Track\NE2h7-220720-093937'} ; ...%(done)
    {'R:\DANEHippocampalResponse\NE2h7\Linear Track\NE2h7-220721-082011'} ; ....%(done)
    
       %{'R:\DANEHippocampalResponse\NE2h8\LinearTrack\NE2h8-220622-100042'} ; ...%(done)
  %  {'R:\DANEHippocampalResponse\NE2h8\LinearTrack\NE2h8-220623-121635'} ; ...%(done)
    %{'R:\DANEHippocampalResponse\NE2h8\LinearTrack\NE2h8-220630-115938'} ; ...%(done)
    
    
    
    {'R:\DANEHippocampalResponse\NE2h9\Linear Track\NE2h9-220720-102405'} ; ...%(done)
    %{'R:\DANEHippocampalResponse\NE2h9\Linear Track\NE2h9-220721-093903'} ; ...%(done)
    ];
...
for i = 1:length(dirs)
    ...
    if exist('sessiondata.mat')
        ...
        v = load('noveltySignal.mat'); %novel object timing
        ...
        for j = 1:length(rewts)
          ...
          % get time to NO
        time2NOt = nan(length(sessiondata.behavior.ts_video),1);
        NOts = cell2mat(v.data(:,2));
        for j = 1:length(NOts)
            if j==1
                kp = sessiondata.behavior.ts_video<NOts(j);
            else
                kp = sessiondata.behavior.ts_video<NOts(j) & sessiondata.behavior.ts_video>NOts(j-1);
            end
            time2NOt(kp) =   NOts(j) - sessiondata.behavior.ts_video(kp);
        ...
        for j = 1:length(rewts)
          ...
          % get time from  NO
        timefromNOt = nan(length(sessiondata.behavior.ts_video),1);
        ...
        for j = 1:length(NOts)
            ...
            timefromNOt(kp) =   sessiondata.behavior.ts_video(kp) -  NOts(j);
        ...
        if length(kp_Novelt) ~= length(time2rewt)
            ...
            time2rew = [time2rew;time2rewt];
             time2NO = [time2NO;time2NOt];
             timefromNO = [timefromNO;timefromNOt];
