...
% reaching the specified convegence tolerance. Let's increase the maximum
...
% same solution (even without resetting the random number generator).
...
% You can also time the method by passing the |'trace'| parameter value
% of |true|. This will return the time it took to run the method in the
% output structure as well as the fit (relative error is one minus the fit)
% at each iteration.
rng('default') % Reproducibility
