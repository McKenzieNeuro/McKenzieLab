...
- New optimization solver [libraries/compact](libraries/compact). The interface to the solver is implemented in [`tt_opt_compact`](tt_opt_compact.m), and used for CP. (!92) The limited-memory compact representation is from
  * Reference: Brust, J.J. (2024, March).
    Useful Compact Representations for Data-Fitting
    arXiv preprint arXiv:2403.12206.
- In [`ktensor/viz`](@ktensor/viz.m) added option for a global title for the entire figure. Added option to specify x-coordinates (rather than assuming 1:N). Also fixed minor bug in the display of the zero line.
- Made CP methods more consistent in that they can all now accept both cell arrays and ktensors as an initial guess. Also created documentation for CP-NMU and created tests for CP-ALS, CP-APR, CP-ARLS, CP-NMU, and CP-OPT. (!93)
- Updates to modern MATLAB class definitions, which allows for derived classes. 
- Fixed bug in [`ktensor/arrange`](@ktensor/arrange.m) to ensure the weights are absorbed into the requested factor matrix.
- Fixed bug in [`create_problem.m`](create_problem.m) that occurs when creating a Tucker tensor with a 1x1x1 core. If we don't make the size of the core explicit in calling the constructor, it loses any trailing zeros.
- Fixed bug in sptensor/isequal wherein it assumed the subs were sorted the same way for 2 sptensor inputs.
- Updated tensor subsref function to be more modern in handling dot subreferences.
- Updated mex file for lbfgsb code to be compatible with newer compilers.
- Various fixes in documentation (!90)
- Moved expensive tests into a different directory so that they don't run automatically. These are the tests for CP_ARLS_LEV and CP_ORTH_ALS. They now live in `tests/special_tests`.
- Added a few function signatures in ['functionSignatures.json'](functionSignatures.json) so that hints work in MATLAB.
- Minor changes in [doc/ttb.xsl](doc/ttb.xsl) to match updates in MATLAB.
